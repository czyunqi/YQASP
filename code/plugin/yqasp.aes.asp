...
'## Description :   Encrypt strings with AES in YQAsp
'##
'######################################################################
Class YQAsp_AES
	...
	Function Escape(ByVal ss)
		If YQasp.isN(ss) Then Escape = "" : Exit Function
		Dim i,c,a,s : s = ""
		For i = 1 To Len(ss)
			c = Mid(ss,i,1)
			a = ASCW(c)
	...
	Function UnEscape(ByVal ss)
		If YQasp.isN(ss) Then UnEscape = "" : Exit Function
		Dim x, s
		x = InStr(ss,"%")
		s = ""
		Do While x>0
