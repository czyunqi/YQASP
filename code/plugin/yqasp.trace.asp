...
'## YQasp.trace.asp
'## ------------------------------------------------------------------------------
'##  Feature      : YQAsp Variable Tracing Plugin
'##  Version      : v1.3
'##  For YQAsp  :  3.0+
...
'##       YQAsp List对象、Connection对象等，使用方法如下：
'##         YQasp.Ext("Trace")(variable)
'##       同时也可以用此方法输出当前的环境变量信息，使用方法如下：
'##         YQasp.Ext("Trace")(":get")    - 输出Request.QueryString变量
'##         YQasp.Ext("Trace")(":post")   - 输出Request.Form变量
'##         YQasp.Ext("Trace")(":cookie") - 输出Request.Cookies变量
'##         YQasp.Ext("Trace")(":server") - 输出Request.ServerVariables变量
'##         YQasp.Ext("Trace")(":session")- 输出当前Session
'##         YQasp.Ext("Trace")(":app")    - 输出已缓存的Application
'##       使用本插件还可以查看数据库结构信息，使用方法如下：
'##         YQasp.Ext("Trace")(":db")     - 查看当前数据库的数据表和视图信息
'##         YQasp.Ext("Trace")(":db.表名") - 查看某一数据表的详细信息
'##       部分项目如要查看详细数据信息，还可使用：
'##         YQasp.Ext("Trace").TraceAll(variable)
...
Class YQAsp_Trace
	...
	Private Sub Class_Initialize()
		s_author	= "coldstone"
		s_version	= "1.0"
		Set tpl = YQasp.Tpl.New
		tpl.TagMask = "{{*}}"
		tpl.LoadStr "<style>.YQasp-trace{width:90%;font-size:12px;font-family:Consolas;margin:10px auto;padding:0;background-color:#FFF;}.YQasp-trace h3,.YQasp-trace h4{font-size:12px;margin:0;line-height:24px;text-align:center;background-color:#999;border:1px solid #555;color:#FFF;border-bottom:none;}.YQasp-trace h4{padding:5px;line-height:1.5em;text-align:left;background-color:#FFC;color:#000; font-weight:normal;}.YQasp-trace h4 strong{color:red;}.YQasp-trace table{width:100%;margin:0;padding:0;border-collapse:collapse;border:1px solid #555;border-bottom:none;}.YQasp-trace th{background-color:#EEE;white-space:nowrap;}.YQasp-trace thead th{background-color:#CCC;}.YQasp-trace th,.YQasp-trace td{font-size:12px;border:1px solid #999;padding:4px;word-break:break-all;}.YQasp-trace span.info{color:#F30;}</style><div class=""YQasp-trace""><h3>YQAsp变量调试</h3><h4>此次调试变量的类型是 <strong>{{type}}</strong> ，{{#if @count!=''}}共有 <strong>{{count}}</strong> 条数据，{{/#if}}以下是其中的{{#if @top>0}}前 <strong>{{top}}</strong> 条，如果要查看全部数据，请使用YQasp.Ext(""Trace"").TraceAll方法{{#else}}数据{{/#if}}：</h4>{{table}}</div>"
		tpl "top", 0
	End Sub
		...
		Select Case VarType(o)
			...
			Case vbString
				If TypeName(o) = "Connection" Then
						...
						tpl "info", "您可以用 YQasp.Ext(""Trace"").TraceAll 方法查看该连接对象全部表和视图的详细信息。"
					...
					Select Case Lcase(o)
								...
								For Each i In Request.Cookies
									If Request.Cookies(i).HasKeys Then
										For Each j In Request.Cookies(i)
											...
											tpl "value", YQasp.Str.HtmlEncode(YQasp.Cookie(i&">"&j))
											tpl.Update "loop"
											s = s + 1
										Next
									Else
										tpl "no", s+1
										tpl "name", "Cookies("""&i&""")"
										tpl "value", YQasp.Str.HtmlEncode(YQasp.Cookie(i))
										tpl.Update "loop"
										s = s + 1
									End If
								Next
						...
						Case ":get", ":querystring"
							...
							Else
								...
								For Each i In Request.QueryString
									...
									tpl "value", YQasp.Str.HtmlEncode(Request.QueryString(i))
									tpl.Update "loop"
								Next
							End If
						Case ":post", ":form"
							...
							Else
								...
								For Each i In Request.Form
									tpl "name", "Form("""&i&""")"
									tpl "value", YQasp.Str.HtmlEncode(Request.Form(i))
									tpl.Update "loop"
								Next
							End If
						Case ":server", ":servervariables"
							...
							For Each i In Request.ServerVariables
								tpl "name", i
								tpl "value", YQasp.Str.HtmlEncode(Request.ServerVariables(i))
								tpl.Update "loop"
							Next
						...
						Case ":session"
							...
							Next
						Case ":db"
						  Set dbtmp = YQasp.db.GetConn()
							TraceDb dbtmp, t
							If dbtmp.State = 0 Then dbtmp.Open
						Case Else
							If YQasp.Str.Test(o,"^:db\.(.+)$") Then
						    Set dbtmp = YQasp.db.GetConn()
								s = YQasp.Str.Replace(o,"^:db\.(.+)$","$1")
								tpl "type", "数据表"
							...
							Else
								tpl.TagStr "table", GetTable(4)
								tpl "type", "字符串"
								tpl "Value", YQasp.Str.HtmlEncode(o)
							End If
					End Select
				End If
			Case vbObject
				Select Case TypeName(o)
							...
							Else
								...
								If o.RecordCount = 1 Then
									...
									For j = 0 To o.Fields.Count-1
										...
										tpl "value", YQasp.Str.HtmlEncode(o.Fields(j).Value)
										tpl.Update "loop"
									Next
								Else
									...
									If t = 0 Then
										...
										tpl "cno", "序号"
										tpl.Tag("top") = YQasp.IIF(o.RecordCount>30, 30, 0)
										For j = 0 To o.Fields.Count-1
					...
					Case "Dictionary"
						...
						Else
							...
							tpl "cvalue", "键值"
							tpl "count", o.Count
							tpl.Tag("top") = YQasp.IIF(o.Count>50 And t=0,50,0)
							For Each i In o
								If t = 0 And j>=50 Then Exit For
								tpl "name", i
								'tpl "value", YQasp.Str.HtmlEncode(o(i))
								ShowValue o(i), "", 0
								tpl.Update "loop"
								j = j + 1
							Next
					...
					Case "YQAsp_List"
						...
						Else
							...
							tpl "cvalue", "键值"
							tpl "count", o.Size
							tpl.Tag("top") = YQasp.IIF(o.Size>50 And t=0,50,0)
							For i = 0 To o.End
								If t = 0 And j>=50 Then Exit For
								tpl "no", i
			...
			Case vbArray,8194,8204,8209
				...
				size1 = Ubound(o)
				size2 = Ubound(o,2)
				arrType = YQasp.IIF(Err.Number=0,2,1)
				On Error Goto 0
				If arrType = 1 Then
				'一维数组
					tpl "type", "数组"
					...
					Else
						...
						tpl "cvalue", "元素值"
						tpl "count", size1+1
						tpl.Tag("top") = YQasp.IIF(size1>49 And t=0,50,0)
						For i = 0 To size1
							If t = 0 And j>=50 Then Exit For
							tpl "name", i
							ShowValue o(i), "", 0
				...
				'二维数组
					...
					If t = 0 Then
						tpl.TagStr "table", GetTable(2)
						tpl "cno", "一维\二维"
						tpl.Tag("top") = YQasp.IIF(size2>19, 20, 0)
						For j = 0 To size1
							tpl "field", "&nbsp;&nbsp;&nbsp;" & j & "&nbsp;&nbsp;&nbsp;"
							tpl.Update "col"
						Next
						For i = 0 To size2
							...
							tpl "i", i
							For j = 0 To size1
								'tpl "value", YQasp.Str.HtmlEncode(o(j,i))
								ShowValue o(j,i), "", 0
								tpl.Update "fields"
							Next
							tpl.Update "rs"
	...
	Private Function ShowValue(ByVal o, ByVal t, ByVal i)
		If IsObject(o) Then
			tpl.Tag("value") = "<span class=""info"">[ "&TypeName(o)&" Object ]" & YQasp.IfThen(YQasp.Has(t),", 要查看其中的内容，请使用 YQasp.Ext(""Trace"")("&t&"("""&i&"""))") & "</span>"
		ElseIf IsArray(o) Then
			tpl.Tag("value") = "<span class=""info"">[ Array ]" & YQasp.IfThen(YQasp.Has(t),", 要查看其中的内容，请使用 YQasp.Ext(""Trace"")("&t&"("""&i&"""))") & "</span>"
		Else
			tpl "value", YQasp.Str.HtmlEncode(o)
		End If
	End Function
	...
	Private Sub TraceDb(ByVal con, ByVal isall)
		...
		If isall = 0 Then
			...
			tpl "cname", "名称"
			tpl "cvalue", "字段名"
			tpl "info", "您可以用 YQasp.Ext(""Trace"")("":db.表名或视图名"") 查看单表或单视图的详细信息；用 YQasp.Ext(""Trace"").TraceAll("":db"") 查看全部表和视图的详细信息。"
		ElseIf isall = 1 Then
			tpl.TagStr "table", GetTable(6)
		End If
		'Set db = YQasp.db.New
		'db.Conn = con
		dbtype = YQasp.Db.GetType(con)
		For i = 0 To Ubound(arr1,2)
			If isall = 0 Then
				tpl.Tag("no") = "表"
				tpl "name", arr1(2,i)
				'Set f = db.GR(arr1(2,i)&":1","1=-1","")
				YQasp.Var("name") = arr1(2,i)
				Set f = YQasp.Db.Execute(con, "Select Top 1 * From {=name} Where 1=-1")
				s = ""
			...
			ElseIf isall = 1 Then
				...
				tpl "tableorview", "数据表"
				tpl "name", YQasp.IfThen(YQasp.Has(t("TABLE_CATALOG")),"["&t("TABLE_CATALOG")&"].") & YQasp.IfThen(YQasp.Has(t("TABLE_SCHEMA")),"["&t("TABLE_SCHEMA")&"].") & "["&t("TABLE_NAME")&"]"
				While Not t.Eof
					...
					End If
					tpl "nullable", YQasp.IfThen(t("IS_NULLABLE"),"√")
	...
	Private Sub TraceTable(ByVal tab, ByRef con)
		...
		tpl "tableorview", YQasp.IIF(dov = "VIEW","视图","数据表")
		tpl "name", YQasp.IfThen(YQasp.Has(t("TABLE_CATALOG")),t("TABLE_CATALOG")&".") & YQasp.IfThen(YQasp.Has(t("TABLE_SCHEMA")),t("TABLE_SCHEMA")&".") & t("TABLE_NAME")
		dbtype = YQasp.Db.GetType(con)
		While Not t.Eof
			tpl "field", t("COLUMN_NAME")
	...
	Private Function GetSQLDataType(ByVal typeid, ByVal flag, ByVal maxlen, ByVal octlen, ByVal numpre, ByVal numscl, ByVal datepre)
		Dim tmp
		Select Case typeid
			Case 2 tmp = "smallint" & YQasp.IfThen(flag=16,",自增")
			Case 3 tmp = "int" & YQasp.IfThen(flag=20,",自增")
			Case 4 tmp = "real"
			Case 5 tmp = "float"
			Case 6 tmp = YQasp.IIF(numpre=10,"smallmoney","money")
			Case 11 tmp = "bit"
			Case 12 tmp = "sql_variant"
			Case 17 tmp = "tinyint" & YQasp.IfThen(flag=16,",自增")
			Case 20 tmp = "bigint" & YQasp.IfThen(flag=16,",自增")
			...
			Case 135 tmp = YQasp.IIF(datepre=0,"smalldatetime","datetime")
			Case Else tmp = "未知类型"
		End Select
		GetSQLDataType = tmp
	End Function
	...
	Private Function GetACCDataType(ByVal typeid, ByVal flag, ByVal maxlen)
		...
		Select Case typeid
			Case 2 tmp = "数字(整型)"
			Case 3 tmp = YQasp.IIF(flag=90,"自动编号/数字(非空长整型)","数字(长整型)")
			Case 4 tmp = "数字(单精度型)"
			Case 5 tmp = "数字(双精度型)"
			Case 6 tmp = "货币"
			Case 7 tmp = "日期/时间"
