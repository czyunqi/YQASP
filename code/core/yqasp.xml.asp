...
'## YQasp.xml.asp
'## -------------------------------------------------------------------
'## Feature     :   YQAsp XML Document Class
'## Version     :   1.0
...
Class YQAsp_Xml
  ...
  Private Sub Class_Initialize()
    ...
    s_xsltPath = ""
    IsOpen = False
    YQasp.Error("error-xml-nocom") = YQasp.Lang("error-xml-nocom")
    YQasp.Error("error-xml-domopen") = YQasp.Lang("error-xml-domopen")
    YQasp.Error("error-xml-domload") = YQasp.Lang("error-xml-domload")
    YQasp.Error("error-xml-savedom") = YQasp.Lang("error-xml-savedom")
    YQasp.Error("error-xml-find") = YQasp.Lang("error-xml-find")
    YQasp.Error("error-xml-notdom") = YQasp.Lang("error-xml-notdom")
    YQasp.Error("error-xml-notnode") = YQasp.Lang("error-xml-notnode")
    YQasp.Error("error-xml-prevel") = YQasp.Lang("error-xml-prevel")
    YQasp.Error("error-xml-nextel") = YQasp.Lang("error-xml-nextel")
    YQasp.Error("error-xml-findel") = YQasp.Lang("error-xml-findel")
  End Sub
  ...
  Private Function NewDom()
    Dim o
    If YQasp.IsInstall("MSXML2.DOMDocument") Then
    'msxml ver 3
      Set o = Server.CreateObject("MSXML2.DOMDocument")
    ElseIf YQasp.IsInstall("Microsoft.XMLDOM") Then
    'msxml ver 2
      Set o = Server.CreateObject("Microsoft.XMLDOM")
    Else
      If YQasp.Debug Then
        YQasp.Error.FunctionName = "YQasp.Xml.NewDom"
        YQasp.Error.Raise "error-xml-nocom"
      End If
      Exit Function
    End If
    '保留空格
  ...
  Public Function Open(byVal f)
    ...
    Open = False
    If YQasp.IsN(f) Then Exit Function
    Set Dom = NewDom()
    '转换为绝对路径
    f = absPath(f)
    '读取文件
    ...
    Else
      If YQasp.Debug Then
        YQasp.Error.FunctionName = "YQasp.Xml.Open"
        YQasp.Error.Detail = f
        YQasp.Error.SetErrors Err, Null, Dom.ParseError
        YQasp.Error.Raise "error-xml-domopen"
      End If
      Set Doc = Nothing
      Set Dom = Nothing
    End If
  ...
  Public Sub Load(ByVal s)
    On Error Resume Next
    If YQasp.IsN(s) Then Exit Sub
    Dim string
    '如果是外部网址则用Http取回,如要指定编码可加在http前，例：gbk>http://....
    If YQasp.Str.Test(s,"^([\w\d-]+>)?https?://") Then
      Dim h : Set h = YQasp.Http.New
      string = h.Get(s)
      Set h = Nothing
    Else
      string = s
    ...
    Else
      If YQasp.Debug Then
        YQasp.Error.FunctionName = "YQasp.Xml.Load"
        YQasp.Error.Detail = Left(s, 30) & "..."
        YQasp.Error.SetErrors Err, Null, Dom.ParseError
        YQasp.Error.Raise "error-xml-domload"
      End If
      Set Doc = Nothing
      Set Dom = Nothing
    End If
...
'  End Property
  ...
  Public Sub [Save]()
    ...
    Else
      If YQasp.Debug Then
        YQasp.Error.FunctionName = "YQasp.Xml.Save"
        YQasp.Error.Detail = s_filePath
        YQasp.Error.Raise "error-xml-savedom"
      End If
    End If
  End Sub
  '另存为(Load进来的只能用此方法保存)
  '可以在保存的文件名后加如 >gbk 来指定保存的编码，如果不指定则：有编码申明的不改变，无编码申明的采用YQasp.Charset值作编码
  Public Sub SaveAs(ByVal p)
    Dim ch,cha,pi
    If Instr(p,">")>0 Then
      ch = YQasp.Str.GetValue(p,">")
      p = YQasp.Str.GetName(p,">")
    End If
    cha = YQasp.IfHas(ch,YQasp.CharSet)
    p = absPath(p)
    '如果没有文档类型申明就加上
    ...
    If Dom.FirstChild.BaseName<>"xml" Then
      Dom.InsertBefore pi, Dom.FirstChild
    Else
      If YQasp.Has(ch) Then Dom.ReplaceChild pi, Dom.FirstChild
    End If
    Dom.Save(p)
    Set pi = Nothing
  End Sub
  ...
  Public Sub SaveAsXHTML(ByVal p, ByVal xsl)
    Dim x,f : Set x = [New]
    If YQasp.Str.Test(xsl,"^([\w\d-]+>)?https?://") Then
      x.Load xsl
    Else
      x.Open xsl
    End If
    f = Dom.TransformNode(x.Dom)
    YQasp.Fso.CreateFile p, f
    Set x = Nothing
  End Sub
  
  '建立新的YQasp Node对象
  Public Function NewNode(ByVal o)
    Set NewNode = New YQAsp_Xml_Node
    NewNode.Dom = o
  End Function
  ...
  Public Default Function Find(ByVal t)
    Dim o,s
    '如果是Html代码片断
    If YQasp.Str.Test(t,"^<[\s\S]+>$") Then
'      Dim n,a,v,r
'      r = "^<([^\s>]+)\s([^>]+)>([\s\S]+)</\1>$"
'      n = YQasp.Str.Replace(t,r,"$1")
'      a = YQasp.Str.Replace(t,r,"$2")
'      v = YQasp.Str.Replace(t,r,"$3")
    Else
      If YQasp.Str.Test(t, "[, >\[@:]") Then
        '按简单表达式取元素
        Set o = Dom.selectNodes(YQasp_Xml_TransToXpath(t))
      Else
        '从标签取元素
    ...
    If o.Length = 0 Then
      If YQasp.Debug Then
        YQasp.Error.FunctionName = "YQasp.Xml.Find"
        YQasp.Error.Detail = t
        YQasp.Error.SetErrors Err, Null, Dom.ParseError
        YQasp.Error.Raise "error-xml-find"
      End If
      Set Find = Nothing
    '如果只有一个元素
  ...
  Public Function Create(ByVal n, ByVal v)
    ...
    If Instr(n," ")>0 Then
      cd = LCase(YQasp.Str.GetValue(n," "))
      n = YQasp.Str.GetName(n," ")
    End If
    '创建注释节点
    If cd="comment" Then
...
Class YQAsp_Xml_Node
  ...
  Public Property Let Dom(ByVal o)
    ...
    Else
      If YQasp.Debug Then
        YQasp.Error.FunctionName = "Xml.Node.Dom"
        YQasp.Error.Raise "error-xml-notdom"
      End If
      Set o_node = Nothing
    End If
  End Property
  ...
  Public Default Function Item(ByVal n)
    ...
    Else
      If YQasp.Debug Then
        YQasp.Error.FunctionName = "Xml.Node.Item"
        YQasp.Error.Detail = TypeName(o_node)
        YQasp.Error.Raise "error-xml-notnode"
      End If
      Set Item = Nothing
    End If
  End Function
  ...
  Public Property Let Text(ByVal v)
    If IsNode Then
      If YQasp.Has(v) Then o_node.Text = v
    '如果是集合则设置每个子节点的文本
    ElseIf IsNodes Then
      Dim i
      For i = 0 To Length - 1
        If YQasp.Has(v) Then o_node(i).Text = v
      Next
    End If
  End Property
  ...
  Public Function Prev
    ...
    Else
      If YQasp.Debug Then
        YQasp.Error.FunctionName = "Xml.Node.Prev"
        YQasp.Error.Detail = o_node.NodeName
        YQasp.Error.Raise "error-xml-prevel"
      End If
      Set Prev = Nothing
    End If
    Set o = Nothing
  ...
  Public Function [Next]
    ...
    Else
      If YQasp.Debug Then
        YQasp.Error.FunctionName = "Xml.Node.Next"
        YQasp.Error.Detail = o_node.NodeName
        YQasp.Error.Raise "error-xml-nextel"
      End If
      Set [Next] = Nothing
    End If
    Set o = Nothing
  ...
  Public Function Find(ByVal t)
    ...
    If YQasp.Str.Test(t, "[, >\[@:]") Then
      '按简单表达式取元素
      Set o = o_node.selectNodes(YQasp_Xml_TransToXpath(t))
    Else
      '从标签取元素
    ...
    If o.Length = 0 Then
      If YQasp.Debug Then
        YQasp.Error.FunctionName = "Xml.Node.Find"
        YQasp.Error.Detail = Array(o_node.NodeName, t)
        YQasp.Error.Raise "error-xml-findel"
      End If
      Set Find = Nothing
    ElseIf o.Length = 1 Then
      Set Find = [New](o(0))
  ...
  Private Function GetNodeDom(ByVal o)
    Select Case TypeName(o)
      ...
      Case "YQAsp_Xml_Node" Set GetNodeDom = o.Dom
    End Select
  End Function
  '添加子节点
  ...
  End Sub
End Class
Function YQasp_Xml_TransToXpath(ByVal s)
  s = YQasp.Str.Replace(s, "\s*,\s*", "|//")
  s = YQasp.Str.Replace(s, "\s*>\s*", "/")
  s = YQasp.Str.Replace(s, "\s+", "//")
  s = YQasp.Str.Replace(s, "(\[)([a-zA-Z]+\])", "$1@$2")
  s = YQasp.Str.Replace(s, "(\[)([a-zA-Z]+[!]?=[^\]]+\])", "$1@$2")
  s = YQasp.Str.Replace(s, "(?!\[\d)\]\[", " and ")
  s = Replace(s, "|", " | ")
  YQasp_Xml_TransToXpath = "//" & s
End Function
%>