...
'## YQasp.upload.asp
'## -------------------------------------------------------------------
'## Feature   :  YQAsp Upload Files Class
'## Version     :   1.0
...
Class YQAsp_MoLibUpload
  ...
  Public IsUploaded, FormArray, s_errLang 'added by YQAsp
  '设置允许上传的总大小
  ...
  Private Sub Class_Initialize()
    ...
    Set FormArray = Server.CreateObject("Scripting.Dictionary") 'added by YQAsp
  ...
  Private Sub Class_Terminate()
    ...
    Next
    Fils.RemoveAll()
    Set FormArray = Nothing 'added by YQAsp
    Set Form = Nothing
    Set Fils = Nothing
  ...
  Public Function GetData()
    ...
    ef = false
    If checkEntryType = false Then ef = true : mvarDescription = YQasp.Lang("error-uplaod-enctypeor-" & s_errLang)
    If vServerVersion>=6 Then
      If Not ef Then
        If vMaxSize > 0 And TotalBytes > vMaxSize Then ef = true : mvarDescription = YQasp.Lang("error-uplaod-filemaxsize-" & s_errLang)
      End If
    End If
        ...
        If Trim(FileName) <> "" Then
          ...
          If vExe <> "" Then
            If checkExe(fileExe) = True Then
              mvarDescription = YQasp.Lang("error-uplaod-filetype-" & s_errLang) & "(." & ucase(fileExe) & ")"
              vErrExe = fileExe
              tempdata = empty
              Exit function
            End If
          ...
          If vSingleSize > 0 And (valueend - formend - 6) > vSingleSize Then
            mvarDescription = YQasp.Lang("error-uplaod-filesize-" & s_errLang)
            tempdata = empty
            Exit function
          End If
          If vMaxSize > 0 And vTotalSize > vMaxSize Then
            mvarDescription = YQasp.Lang("error-uplaod-filemaxsize-" & s_errLang)
            tempdata = empty
            Exit function
          End If
    ...
    For Each postKey In FormArray
      YQasp.Var("post." & postKey) = FormArray(postKey).GetArray()
    Next
    IsUploaded = True
    'added by YQAsp end
    GetData =true
  ...
  Public Function Save(Byref Name,byval tOption, byval OverWrite)
    ...
    if not isobject(name) then
      ...
      If Not File.IsFile Then
        File.Succeed = false
        File.Exception = YQasp.Lang("error-uplaod-fileno-" & s_errLang)
        Set Save = File
        Exit Function
      End If
    else
