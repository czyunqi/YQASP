...
'## YQasp.tar.asp
'## -------------------------------------------------------------------
'## Feature     :   YQAsp File Archiver Class
'## Version     :   1.0
...
Class YQAsp_Tar
  ...
  Private Sub Class_Initialize()
    ...
    b_hasSelf = True
    s_downFileName = ""
    YQasp.Error("error-tar-packfaild") = YQasp.Lang("error-tar-packfaild")
    YQasp.Error("error-tar-unpackfaild") = YQasp.Lang("error-tar-unpackfaild")
  End Sub
  ...
  Public Property Let HasSelf(ByVal b)
    b_hasSelf = YQasp.IIF(b, True, False)
  End Property
  ...
  Public Sub AddTo(ByVal s_path, ByVal s_insidePath)
    ...
    s_path = YQasp.Fso.MapPath(s_path)
    s_fName = Mid(s_path, InStrRev(s_path, "\")+1)
    'YQasp.Console "Add Folder: " & s_path
    If YQasp.Fso.isFolder(s_path) Then
      '如果是文件夹
      LoadFolder s_path, s_insidePath & YQasp.IfThen(b_hasSelf, s_fName & "/")
    ElseIf YQasp.Fso.IsFile(s_path) Then
      '如果是文件
      dic_files(s_insidePath & s_fName) = Array(True, True, s_path, GetFileTimeStamp(s_path))
    Else
      If YQasp.Debug Then
        YQasp.Error.FunctionName = "YQasp.Tar.AddTo"
        YQasp.Error.Detail = s_path
  ...
  Private Sub LoadFolder(ByVal s_path, ByVal s_insidePath)
    ...
    a_folder = YQasp.Fso.Dir(s_path) '读取文件夹
    For i = 0 To Ubound(a_folder, 2)
      s_file = s_path & "\" & a_folder(0, i)
      s_time = YQasp.Date.ToUnixTimeCn(a_folder(2, i))
  ...
  Private Function GetFileTimeStamp(ByVal s_path)
    ...
    GetFileTimeStamp = YQasp.Date.ToUnixTimeCn(f.DateLastModified)
    Set f = Nothing
  End Function
  '添加文件夹或文件
  ...
  Public Function CreateFolder(ByVal s_path)
    ...
    For i = 0 To Ubound(arr)
      If YQasp.Has(arr(i)) Then
        tmp = tmp & arr(i) & "/"
        If Not dic_files.Exists(tmp) Then
          dic_files(tmp) = Array(False, False, YQasp.Date.GetTimeStamp())
        End If
      End If
    Next
    CreateFolder = tmp
  ...
  Public Sub CreateFile(ByVal s_fileName, ByVal s_content)
    ...
    End If
    dic_files(s_fileName) = Array(True, False, s_content, YQasp.Date.GetTimeStamp())
  End Sub
  '打包
  ...
  Public Function PackTo(ByVal s_filePath)
    ...
    If YQasp.Has(s_filePath) Then
      s_savePath = s_filePath
      p = YQasp.Fso.MapPath(s_filePath)
      Call YQasp.Fso.MD(Left(p,InstrRev(p,"\")-1))
    End If
    ...
    With o_tar
      ...
      If YQasp.Has(s_filePath) Then
        .SaveToFile p, 2
      Else
        OutPutFile o_tar, YQasp.IfHas(s_downFileName, "file_" & YQasp.Date.Format(Now, "ymmddhhiiss") & ".tar")
      End If
      .Close
    End With
    Set o_tar = Nothing
    If Err.Number <> 0 Then
      PackTo = False
      If YQasp.Debug Then
        YQasp.Error.FunctionName = "YQasp.Tar.PackTo"
        YQasp.Error.Detail = p
        YQasp.Error.Raise "error-tar-packfaild"
      End If
    Else
      PackTo = True
    End If
  ...
  Private Sub AddToTar(ByRef o_tar, ByVal s_pathname, ByRef o_file, ByVal timeStamp)
    ...
    With o_tar
      pos = .Position
      '文件名
      Set o_filename = LoadText(s_pathname & String(100-YQasp.Str.Leng(s_pathname), Chr(0)), "gbk")
      o_filename.CopyTo o_tar
      Set o_filename = Nothing
      '类型
      .WriteText YQasp.IIF(is_folder, " 4", "10")
      ...
      .WriteText "       " & Chr(0)
      '链接
      .WriteText YQasp.IIF(is_folder, "5", "0")
  ...
  Public Function UnPackTo(ByVal s_tarFilePath, ByVal s_folderPath)
    ...
    If YQasp.IsN(s_folderPath) Then s_folderPath = "."
    s_savePath = s_folderPath
    s_folderPath = YQasp.Fso.MapPath(s_folderPath)
    If b_hasSelf Then
      s_folderPath = s_folderPath & "\" & YQasp.Fso.NameOf(s_tarFilePath)
      s_savePath = s_savePath & YQasp.IfThen(Right(s_savePath, 1) <> "/", "/") & YQasp.Fso.NameOf(s_tarFilePath)
    End If
    YQasp.Fso.MD s_folderPath
    ...
    With o_tar
      ...
      Do While(True)
        ...
        If Right(s_fileName, 1) = "\" And Right(s_type, 2) = "40" Then
          '如果是文件夹
          YQasp.Fso.MD s_folderPath & "\" & Left(s_fileName, Len(s_fileName)-1)
          'YQasp.Console s_folderPath & "\" & Left(s_fileName, Len(s_fileName)-1)
          .ReadText(32)
        Else
          '如果是文件，取出文件大小
          i_fileSize = Int("&0" & Int(Right(.ReadText(32), 11)))
          'YQasp.Console s_folderPath & "\" & s_fileName & "|" & i_fileSize
          is_file = True
        End If
        '跳过剩余的文件头
        .ReadText(377)
        ...
        If is_file Then
          ...
          If i_fileSize > 0 Then
            YQasp.Fso.SaveAs s_folderPath & "\" & s_fileName, o_data.Read(i_fileSize)
            .ReadText(i_fileSize)
          Else
            YQasp.Fso.CreateFile s_folderPath & "\" & s_fileName, ""
          End If
        End If
        '跳过所有的填充块
    ...
    If Err.Number <> 0 Then
      UnPackTo = False
      If YQasp.Debug Then
        YQasp.Error.FunctionName = "YQasp.Tar.UnPackTo"
        YQasp.Error.Detail = p
        YQasp.Error.Raise "error-tar-unpackfaild"
      End If
    Else
      UnPackTo = True
    End If
  ...
  Private Function LoadFile(ByVal s_path)
    ...
    LoadFile.Open
    LoadFile.LoadFromFile YQasp.Fso.MapPath(s_path)
    If Err.Number <> 0 Then
      If YQasp.Debug Then
        YQasp.Error.FunctionName = "YQasp.Tar.UnPackTo"
        YQasp.Error.Detail = s_path
  ...
  Private Function LoadText(ByVal s_content, ByVal charset)
    If YQasp.IsN(charset) Then charset = YQasp.CharSet
