...
Class YQAsp_Console
  ...
  Public Default Sub Log(ByVal message)
    ...
    If Enable Then
      '将输入的对象转为文本
      string = YQasp.Str.ToString(message)
  ...
  Public Sub Out()
    ...
    Dim s_tmp
    '检查token值是否正确
    If YQasp.Has(s_token) And Not YQasp.Str.IsEqual(YQasp.Get("token"), s_token) Then
      YQasp.Print "invalidToken"
    Else
      '如果token正确则输出缓存内容到控制台
      YQasp.Print Escape(YQasp.Str.HtmlEncode(Session(Name)))
      Session(Name) = Empty
    End If
  End Sub
End Class
