...
'## YQasp.str.asp
'## -------------------------------------------------------------------
'## Feature     :   YQAsp String Class
'## Version     :   1.0
'## Author      :   云奇(114066164@qq.com)
'## Update Date :   2021-7-15
'## Description :   YQAsp String Class
'##
'######################################################################
...
Class YQAsp_String
  ...
  Private Sub Class_Initialize()
    Set o_re = New YQAsp_StringOriginal
  End Sub
  ...
  Private Function FormatString(ByVal s, ByRef v, ByVal t)
      ...
      Case 9
        Select Case TypeName(v)
          ...
          Case "Recordset"
            ...
            Next
          '字典
          Case "Dictionary", "YQAsp_Json_Object"
            If TypeName(v) = "YQAsp_Json_Object" Then Set v = v.GetDictionary
            For Each k In v
    ...
    If Instr(s, "{=") Then
      ...
      For Each m In matches
        s = o_re.ReCase(s, m, YQasp.Var(m.SubMatches(0)))
      Next
      Set matches = Nothing
    End If
    s = o_re.Re(s,Chr(1),"{")
        ...
        Select Case Left(kind,1)
          ...
          Case "N"
            If isNumeric(v) Then
              ...
              If YQasp.IsN(style) And YQasp.IsN(deci) Then
                s = o_re.ReCase(s, ru, YQasp.IIF(Instr(Cstr(v),".")>0 And v<1,"0" & v,v))
              Else
                deci = YQasp.IfHas(deci,-1)
                If percent Then
          ...
          Case "D"
            If isDate(v) Then
              s = o_re.ReCase(s, ru, YQasp.Date.Format(v,Mid(kind,2)))
            End If
          '转大写
          Case "U"
  ...
  Public Function IsSame(ByVal string1, ByVal string2)
    If YQasp.IsN(string1) And YQasp.IsN(string2) Then IsSame = True : Exit Function
    IsSame = (StrComp(string1, string2, 1) = 0)
  End Function
  '比较文本是否一致（区分大小写）
  Public Function IsEqual(ByVal string1, ByVal string2)
    If YQasp.IsN(string1) And YQasp.IsN(string2) Then IsEqual = True : Exit Function
    IsEqual = (StrComp(string1, string2, 0) = 0)
  End Function

  '比较两个字符串的大小，区分大小写
  '返回： Boolean值
  '用法： YQasp.Print Compare("ABCD", "<", "abcd")  'True
  ...
  '返回：Boolean值
  '说明：If YQasp.Str.IsInList("A,B,C", str) Then
  '     等同于下面的语句并且忽略大小写：
  '     If str = "A" Or str = "B" Or str = "C" Then
  Public Function IsInList(ByVal string, ByVal str)
    Dim s1, s2
    If YQasp.Has(str) Then
      s1 = YQasp.IIF(Left(string,1)=",", string, "," & string)
      s1 = YQasp.IIF(Right(s1,1)=",", s1, s1 & ",")
      s2 = YQasp.IIF(Left(str,1)=",", str, "," & str)
      s2 = YQasp.IIF(Right(s2,1)=",", s2, s2 & ",")
      IsInList = IsIn(s1, s2)
    End If
  End Function
  ...
  Private Function GetNameValue_(ByVal string, ByVal separator, ByVal isRev)
    Dim n, arr(1)
    n = YQasp.IIF(isRev, InstrRev(string, separator), Instr(string, separator))
    If n > 0 Then
  ...
  Public Function Cut(ByVal s, ByVal strlen)
    If YQasp.IsN(s) Then Cut = "" : Exit Function
    '去除html标签、换行和制表符
    s = HtmlFilter(s)
    s = o_re.Re(s, vbCrLf, "")
    s = o_re.Re(s, vbTab, "")
    If YQasp.IsN(strlen) Then Cut = s : Exit Function
    ...
    t = 0
    For i = 1 To Len(s)
      t = YQasp.IIF(Abs(Ascw(Mid(s,i,1)))>255, t+2, t+1)
      If t >= i_leng Then
        Cut = Left(s, i) & s_sep
        Exit For
      End If
  ...
  Public Function Leng(string)
    ...
    For i = 1 To Len(string)
      n = YQasp.IIF(Abs(Ascw(Mid(string,i,1)))>255, n+2, n+1)
    Next
    Leng = n
  End Function
  '不区分大小写替换
  ...
  Public Function Replace(ByVal string, ByVal rule, ByVal replaceWith)
    Replace = YQasp_Replace(string, rule, replaceWith, False)
  End Function
  '正则替换多行模式
  Public Function ReplaceLine(ByVal string, ByVal rule, ByVal replaceWith)
    ReplaceLine = YQasp_Replace(string, rule, replaceWith, True)
  End Function
  '替换正则表达式编组
  '说明：按正则表达式的规则替换一个字符串中某个捕获编组的内容
  '示例：YQasp.Str.ReplacePart("photo-3.html", "^(\w+)-(\d+)\.html$", "$2", "4")
  '     返回： photo-4.html
  Public Function ReplacePart(ByVal string, ByVal rule, ByVal group, ByVal replaceWith)
    If Not YQasp_Test(string, rule) Then
      '如果规则不匹配则直接返回字符串
      ReplacePart = string
      Exit Function
    End If
  ...
  Private Function isIDCard(ByRef s)
    ...
    Ai = YQasp.IIF(Len(s) = 18,Mid(s, 1, 17),Left(s, 6) & "19" & Mid(s, 7, 9))
  ...
  End Function
  '正则替换原型
  Private Function YQasp_Replace(ByVal string, ByVal rule, Byval result, ByVal isMultiLine)
    Dim o_regexp
    If YQasp.Has(string) Then
      Set o_regexp = New Regexp
      o_regexp.Global = True
      o_regexp.IgnoreCase = True
      ...
      Set o_regexp = Nothing
    End If
    YQasp_Replace = string
  End Function
  '正则匹配原型
  Private Function YQasp_Test(ByVal s, ByVal p)
    Dim o_regexp
    If YQasp.IsN(s) Then YQasp_Test = False : Exit Function
    Set o_regexp = New Regexp
    o_regexp.Global = True
    o_regexp.IgnoreCase = True
    o_regexp.Pattern = p
    YQasp_Test = o_regexp.Test(CStr(s))
    Set o_regexp = Nothing
  End Function
 ...
 '   string = o_re.Re(string, "&#" & i & "", "")
  ...
  Public Function ToString(ByVal o)
    ...
    Select Case VarType(o)
      ...
      Case 8, 9
        ...
        Select Case TypeName(o)
          Case "Connection"
            ...
            SB.Append o.State
            SB.Append ", ""type"":"""
            SB.Append YQasp.Db.GetType(o)
            SB.Append """, ""version"":"""
            SB.Append YQasp.Db.GetVersion(o)
          ...
          '记录集
            ...
            Else
              ...
              SB.Append "]}"
            End If
          Case "Dictionary", "IRequestDictionary", "IReadCookie", "YQAsp_Json_Object", "Errors", "IMatch2"
          '字典对象
            Dim isString
            ...
            Else
              If TypeName(o) = "YQAsp_Json_Object" Then Set o = o.GetDictionary
              SB.Append "{"
              j = 0
              For Each i In o
                If j > 0 Then SB.Append ", "
      ...
      Case 2,3,4,5,6
        ...
        End If
      Case 11
        SB.Append YQasp.IIF(o, "true", "false")
      Case Else
        SB.Append o
    End Select
  ...
  Private Function QuoteString(ByRef string)
    ...
    Select Case VarType(string)
      ...
      Case 0
        b_quote = True
      Case 8
        'YQasp.Console TypeName(string)
        'YQasp.Console string
        If IsNumeric(string) Then
          ...
          Else
            b_quote = True
          End If
        ElseIf IsInList("String,IStringList,YQAsp_StringObject", TypeName(string)) Then
          b_quote = True
        ElseIf TypeName(string) = "IReadCookie" Then
          'YQasp.Println string.Count
  ...
  Public Function BytesToString(ByVal byt, ByVal char)
    ...
    dim oStrm
    char = YQasp.IfHas(char, YQasp.CharSet)
  ...
  Private Function JMultiArray(ByRef aArray)
    ...
    If dimensions > 0 Then
      ...
      'YQasp.Console "<" & "%" & vbCrlf & b & jexec & vbCrlf & "%" & ">" '//调试生成的语句
      Execute(b & jexec)
    End If
  End Function
  ...
  Public Sub JsAlert(ByVal string)
    YQasp.PrintEnd JavaScript(FormatString("alert('{1}');history.go(-1);",JsEncode(string),1))
  End Sub
  '输出javascript的alert警告框消息并跳转到其他页面
  Public Sub JsAlertUrl(ByVal string, ByVal url)
    YQasp.PrintEnd JavaScript(FormatString("alert('{1}');location.href='{2}';",Array(JsEncode(string),url),1))
  End Sub
  '输出javascript的选择消息框并根据选择跳转到不同的页面
  Public Sub JsConfirmUrl(ByVal string, ByVal yesUrl, ByVal cancelUrl)
    YQasp.PrintEnd JavaScript(FormatString("location.href=confirm('{1}')?'{2}':'{3}';",Array(JsEncode(string),yesUrl,cancelUrl),1))
  End Sub
  ...
  Public Function RandomStr(ByVal string)
    ...
    a = ""
    If YQasp_Test(string, "(<\d+>|<\d+-\d+>)") Then
    '如果参数中包含 <n> 或 <m-n>
      t = string
      p = GetNameValue(string, ":")
      If YQasp.Has(p(1)) Then
        a = p(1) : t = p(0)
      End If
      ...
      Next
      Set reg = Nothing
    ElseIf YQasp_Test(string,"^\d+-\d+$") Then
  ...
  Public Function RandomString(ByVal length, ByVal allowStr)
    Dim i, sb
    If YQasp.IsN(allowStr) Then allowStr = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"
    Set sb = StringBuilder()
    Randomize(Timer)
    For i = 1 To length
  ...
  Public Function StringBuilder()
    Set StringBuilder = New YQAsp_Str_StringBuilder
  End Function
  '表单验证
  Public Function Validate(ByVal string)
    Set Validate = New YQAsp_Validation
    Validate.Source = string
  End Function
  '链式操作
  Public Default Function Text(ByVal obj)
    Set Text = New YQAsp_StringObject
    Text.Value = ToString(obj)
  End Function
End Class
'重写VBS内部函数
Class YQAsp_StringOriginal
