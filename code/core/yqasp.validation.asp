<%
'######################################################################
'## YQasp.validation.asp
'## -------------------------------------------------------------------
'## Feature     :   YQAsp String Validation Class
'## Version     :   1.0
...
Class YQAsp_Validation
  ...
  Public Default Property Get Value()
    If b_return Then
      ...
      For i = 0 To UBound(a_return)
        a_return(i) = CStr(YQasp.IIF(b_validate, YQasp.IfHas(a_return(i), s_default), YQasp.IfHas(a_return(i), YQasp.IfHas(s_default, YQasp.IfHas(s_msg, False)))))
      Next
      If b_array Then
        Value = a_return
      Else
        Dim o : Set o = New YQAsp_ValidationTool
  ...
  Private Function NewValidation()
    Set NewValidation = New YQAsp_Validation
  ...
  Public Function Trim()
    Dim o : Set o = New YQAsp_StringOriginal
  ...
  Public Function Msg(ByVal string)
    ...
    If b_validate Then
      Msg.MsgInfo = s_msg
    Else
      If YQasp.Has(s_msg) Then
        Msg.MsgInfo = s_msg
      Else
        If Instr(string, "%n") Then
         string = Replace(string, "%n", YQasp.IfHas(s_name, YQasp.Lang("val-item")))
        End If
  ...
  Public Function Alert()
    ...
    If Not b_validate Then
      YQasp.Str.JsAlert YQasp.IfHas(s_msg, s_msgDefault)
    End If
  End Function
  ...
  Public Function AlertUrl(ByVal url)
    ...
    If Not b_validate Then
      YQasp.Str.JsAlertUrl YQasp.IfHas(s_msg, s_msgDefault), url
    End If
  End Function
  ...
  Public Function PrintEnd()
    ...
    If Not b_validate Then
      YQasp.PrintEnd YQasp.IfHas(s_msg, s_msgDefault)
    End If
  End Function
  ...
  Public Function PrintEndJson()
    ...
    If Not b_validate Then
      ...
      s_json = s_json & YQasp.Str.JsEncode(YQasp.IfHas(s_msg, s_msgDefault))
      If YQasp.Has(s_field) Then
        s_json = s_json & """, ""validateField"" : """
        s_json = s_json & s_field
      End If
      s_json = s_json & """}"
      YQasp.PrintEnd s_json
    End If
  End Function
  '生成验证规则默认提示信息
  Private Sub CreateMsgDefault(ByRef validation, ByVal langString, ByVal arrValue)
    Dim string, i
    If YQasp.IsN(validation.MsgDefault) Then
      string = YQasp.IfHas(validation.NameString, YQasp.Lang("val-item"))
      If YQasp.Lang.Exists("val-" & langString) Then
        '如果语言包中有相应的内容
        string = string & YQasp.Lang("val-" & langString)
        If YQasp.Has(arrValue) Then
        ...
        End If
      Else
        string = string & YQasp.Lang("val-test")
      End If
  ...
  Public Function Test(ByVal regexpString)
    Dim i, b_val : b_val = True
    For i = 0 To UBound(a_return)
      If YQasp.Has(a_return(i)) And Not YQasp.Str.Test(a_return(i), regexpString) Then
        a_return(i) = Empty
        If b_val Then b_val = False
      End If
    Next
  ...
  Public Function DateBetween(ByVal minDate, ByVal maxDate)
    ...
    b_val2 = True
    For i = 0 To UBound(a_return)
      If YQasp.Has(a_return(i)) And Not YQasp.Str.Test(a_return(i), "date") Then
        a_return(i) = Empty
        If b_val1 Then b_val1 = False
      End If
      If YQasp.Has(a_return(i)) Then
  ...
  Public Function Between(ByVal min, ByVal max)
    ...
    b_val2 = True
    For i = 0 To UBound(a_return)
      If YQasp.Has(a_return(i)) And Not YQasp.Str.Test(a_return(i), "number") Then
        a_return(i) = Empty
        If b_val1 Then b_val1 = False
      End If
      If YQasp.Has(a_return(i)) Then
  ...
  Public Function LengthBetween(ByVal minNumber, ByVal maxNumber)
    Dim i, b_val : b_val = True
    For i = 0 To UBound(a_return)
      If YQasp.Has(a_return(i)) And Len(a_return(i)) < minNumber Or Len(a_return(i)) > maxNumber Then
        a_return(i) = Empty
        If b_val Then b_val = False
      End If
    Next
  ...
  Public Function Length(ByVal number)
    Dim i, b_val : b_val = True
    For i = 0 To UBound(a_return)
      If YQasp.Has(a_return(i)) And Len(a_return(i)) <> number Then
        a_return(i) = Empty
        If b_val Then b_val = False
      End If
    Next
  ...
  Public Function Same(ByVal string)
    Dim i, b_val : b_val = True
    For i = 0 To UBound(a_return)
      If YQasp.Has(a_return(i)) And Not YQasp.Str.IsSame(a_return(i), string) Then
        a_return(i) = Empty
        If b_val Then b_val = False
      End If
    Next
  ...
  Public Function SamePost(ByVal string)
    ...
    If YQasp.Has(s_source) And Not YQasp.Str.IsSame(s_source, YQasp.Post(string)) Then
  ...
  Public Function SameSession(ByVal string)
    ...
    If Not YQasp.Str.IsSame(s_source, Session(string)) Or YQasp.IsN(s_source) Then
      If YQasp.IsN(s_name) Then
        SameSession.NameString = YQasp.Lang("val-verify")
      End If
