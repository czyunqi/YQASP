...
Class YQAsp_Date
  ...
  Public Function Format(ByVal iTime, ByVal iFormat)
    If YQasp.IsN(iTime) Or Not IsDate(iTime) Then Format = "" : Exit Function
    '调用系统函数格式化时间
    ...
    If isDate(iFormat) Or YQasp.IsN(iFormat) Then
      '如果第二个参数为空，则设定为和现在时间相比较
      If YQasp.IsN(iFormat) Then : iFormat = Now() : pastTime = true : End If
      dire = YQasp.Lang("date-after") : If DateDiff("s",iFormat,iTime)<0 Then : dire = YQasp.Lang("date-ago") : before = True : End If
      ...
      If diffs < 60 Then Format = YQasp.Lang("date-justnow") : Exit Function
      If diffs < 1800 Then Format = Int(diffs\60) & YQasp.Lang("date-minutes")  & dire : Exit Function
      If diffs < 2400 Then Format = YQasp.Lang("date-halfhour")  & dire : Exit Function
      If diffs < 3600 Then Format = Int(diffs\60) & YQasp.Lang("date-minutes")  & dire : Exit Function
      If diffs < 259200 Then
        If diffd = 3 Then Format = YQasp.Lang("date-3days") & dire & " " & iiHour & ":" & iiMinute : Exit Function
        If diffd = 2 Then Format = YQasp.IIF(before,YQasp.Lang("date-daybeforeyesterday"), YQasp.Lang("date-dayaftertomorrow")) & iiHour & ":" & iiMinute : Exit Function
        If diffd = 1 Then Format = YQasp.IIF(before,YQasp.Lang("date-yesterday"),YQasp.Lang("date-tomorrow")) & iiHour & ":" & iiMinute : Exit Function
        Format = Int(diffs\3600) & YQasp.Lang("date-hours") & dire : Exit Function
      End If
      If diffd < 7 Then Format = diffd & YQasp.Lang("date-days")  & dire & " " & iiHour & ":" & iiMinute : Exit Function
      '如果第二个参数为空，则只显示2周内的相差时间
      If diffd < 14 Then
        If diffw = 1 Then Format = YQasp.IIF(before,YQasp.Lang("date-lastweek"),YQasp.Lang("date-nextweek")) & iWeek(tWeek) & " " & iiHour & ":" & iiMinute : Exit Function
        If Not pastTime Then Format = diffd & YQasp.Lang("date-days") & dire : Exit Function
      End If
      '如果第二个参数为具体时间，则显示3年内的相差时间
      If Not pastTime Then
        If diffd < 31 Then
          If diffm = 2 Then Format = YQasp.Lang("date-2months") & dire : Exit Function
          If diffm = 1 Then Format = YQasp.IIF(before,YQasp.Lang("date-lastmonth"),YQasp.Lang("date-nextmonth")) & iDay & YQasp.Lang("date-day") : Exit Function
          Format = diffw & YQasp.Lang("date-weeks") & dire : Exit Function
        End If
        If diffm < 36 Then
          If diffy = 3 Then Format = YQasp.Lang("date-3years") & dire : Exit Function
          If diffy = 2 Then Format = YQasp.IIF(before,YQasp.Lang("date-yearbeforelast"),YQasp.Lang("date-yearafternext")) & iMonth & YQasp.Lang("date-month") : Exit Function
          If diffy = 1 Then Format = YQasp.IIF(before,YQasp.Lang("date-last"),YQasp.Lang("date-next")) & iMonth & YQasp.Lang("date-month") : Exit Function
          Format = diffm & YQasp.Lang("date-months") & dire : Exit Function
        End If
        Format = diffy & YQasp.Lang("date-years") & dire : Exit Function
      Else
        '如时间超过上述范围则直接显示
  ...
  Public Function ToUnixTime(ByRef dateTime, ByRef timeZone)
    If YQasp.IsN(dateTime) or Not IsDate(dateTime) Then dateTime = Now
    If YQasp.IsN(timeZone) or Not isNumeric(timeZone) Then TimeZone = 0
  ...
  Public Function FromUnixTime(ByRef timeStamp, ByRef timeZone)
    If YQasp.IsN(timeStamp) Or Not IsNumeric(timeStamp) Then
      FromUnixTime = Now()
      Exit Function
    End If
