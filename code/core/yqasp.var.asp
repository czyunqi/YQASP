<%
'######################################################################
'## YQasp.var.asp
'## -------------------------------------------------------------------
'## Feature     :   YQAsp Variables Class
'## Version     :   1.0
'## Author      :   云奇(114066164@qq.com)
'## Update Date :   2021-7-15
'## Description :   Get and set YQAsp super variables.
'##
'######################################################################

Class YQAsp_Var
  Private o_var
  Private b_loaded
  Private Sub Class_Initialize()
    Set o_var = Server.CreateObject("Scripting.Dictionary")
    o_var.CompareMode = 1 '对key进行文本比较
    b_loaded = False
  End Sub
  Private Sub Class_Terminate()
    Dim key
    For Each key In o_var
      If IsObject(o_var(key)) Then Set o_var(key) = Nothing
    Next
    Set o_var = Nothing
  End Sub
  
  '读取和设置YQAsp超级变量集
  '优先级依次为: 
  '  1.自定义变量
  '  2.Request.QueryString
  '  3.Request.Form
  '  4.YQAsp系统变量
  '  5.Request.ServerVariables
  '  即如有同名的变量，在上一个集合中找到则立即返回值，不再在下一集合中查找
  Public Default Property Get Var(ByVal key)
    Dim s_var
    If Not b_loaded Then Call getVars()
    If o_var.Exists(key) Then
      s_var = o_var(key)
    ElseIf o_var.Exists("get." & key) Then
      s_var = o_var("get." & key)
      If Not isArray(s_var) Then
         If Instr(s_var, "%") > 0 Then s_var = YQasp.UrlDecode(s_var)
      end if
    ElseIf o_var.Exists("post." & key) Then
      s_var = o_var("post." & key)
    ElseIf YQasp.Str.IsSame(key,"YQasp.newid") Then
      s_var = YQasp.NewID()
    ElseIf YQasp.Str.IsSame(key,"YQasp.datetime") Then
      s_var = Now()
    ElseIf YQasp.Str.IsSame(key,"YQasp.date") Then
      s_var = Date()
    ElseIf YQasp.Str.IsSame(key,"YQasp.time") Then
      s_var = Time()
    ElseIf YQasp.Str.IsSame(key,"YQasp.timestamp") Then
      s_var = YQasp.Date.GetTimeStamp()
    ElseIf YQasp.Str.IsSame(key,"YQasp.ip") Then
      s_var = YQasp.GetIP()
    ElseIf o_var.Exists("YQasp." & key) Then
      s_var = o_var("YQasp." & key)
    ElseIf YQasp.Str.StartsWith(key, "server.") Then
      s_var = Request.ServerVariables(Mid(key,8))
    Else
      s_var = ""
    End If
    If Not IsArray(s_var) Then
      If Instr(s_var, "{=") Then
        Dim matches, Match
        Set matches = YQasp.Str.Match(s_var, "\{=(.+?)\}")
        For Each match In matches
          s_var = Replace(s_var, match, Me.Var(match.SubMatches(0)), 1, -1, 1)
        Next
        Set matches = Nothing
      End If
    End If
    Var = s_var
  End Property
  Public Property Let Var(ByVal key, ByVal value)
    If Not b_loaded Then Call getVars()
    Dim i,leng
    If IsArray(value) Then
      leng = UBound(value)
      '如果是数组则：1、将数组转为字符串存入 key
      o_var(key) = Join(value, ", ")
      '2、将原始数组存入 key_array
      o_var(key & "_array") = value
      '3、将数组长度存入 key_array_length
      o_var(key & "_array_length") = leng + 1
      '4、将数组元素存入 key_array_0、key_array_1...
      For i = 0 To leng
        o_var(key & "_array_" & i) = value(i)
      Next
    Else
      o_var(key) = value
    End If
  End Property

  '取得YQAsp变量集原始字典对象
  Public Function [GetObject]()
    If Not b_loaded Then Call getVars()
    Set [GetObject] = o_var
  End Function

  '查找是否包含某一变量
  Public Function Has(ByVal key)
    If Not b_loaded Then Call getVars()
    Has = (o_var.Exists(key) Or o_var.Exists("get." & key) Or o_var.Exists("post." & key) Or YQasp.Str.IsInList("YQasp.newid,YQasp.datetime,YQasp.date,YQasp.time", key) Or o_var.Exists("YQasp." & key))
  End Function
  
  '将页面参数获取值写入到Var集合
  Private Sub getVars()
    ''取表单值
    If Not YQasp.Upload.checkEntryType Then
      Call GetRequest("post")
    End If
    ''取URL参数值
    Call GetRequest("get")
    b_loaded = True
  End Sub
  '取得Post和Get值
  Private Sub GetRequest(ByVal requestType)
    Dim requestDic, requestKey
    Dim key, value, valueCount, values(), i
    If requestType = "get" Then
      Set requestDic = Request.QueryString
    ElseIf requestType = "post" Then
      Set requestDic = Request.Form
    End If
    If requestDic.Count >= 0 Then
      For Each requestKey In requestDic
        key = requestType & "." & LCase(requestKey)
        value =  requestDic(requestKey)
        o_var(key) = value
        '同时存入数组***_array变量
        valueCount = requestDic(requestKey).Count - 1
        key = key & "_array"
        o_var(key & "_length") = valueCount + 1
        ReDim values(valueCount)
        For i = 0 To (valueCount)
          values(i) = requestDic(requestKey)(i+1)
          o_var(key & "_" & i) = values(i)
        Next
        o_var(key) = values
      Next
    End If
  End Sub
End Class
%>