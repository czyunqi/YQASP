...
'## YQasp.list.asp
'## -------------------------------------------------------------------
'## Feature     :   YQAsp List(Array) Class
'## Version     :   1.0
'## Author      :   云奇(114066164@qq.com)
'## Update Date :   2021-7-15
'## Description :   A super Array class in YQAsp
...
Class YQAsp_List
  ...
  Private Sub Class_Initialize
    YQasp.Error("error-list-indexnull") = YQasp.Lang("error-list-indexnull")
    YQasp.Error("error-list-indexwrong") = YQasp.Lang("error-list-indexwrong")
    YQasp.Error("error-list-outofrange") = YQasp.Lang("error-list-outofrange")
    YQasp.Error("error-list-outofhash") = YQasp.Lang("error-list-outofhash")
    YQasp.Error("error-list-notlist") = YQasp.Lang("error-list-notlist")
  ...
  Public Function NewSimpleArray()
    Set NewSimpleArray = New YQAsp_Json_Array
  End Function
  
  '是否忽略大小写
  Public Property Let IgnoreCase(ByVal b)
    i_comp = YQasp.IIF(b, 1, 0)
  End Property
  ...
  Public Property Let At(ByVal n, ByVal v)
    If YQasp.IsN(n) Then
      If YQasp.Debug Then
        YQasp.Error.Raise "error-list-indexnull"
      End If
      Exit Property
    End If
    If YQasp.Str.Test(n,"^\d+$") Then
      '如果是数字就直接添加到数组下标
      If n > [End] Then
      ...
      End If
      a_list(n) = v
    ElseIf YQasp.Str.Test(n,"^[\w\./]+$") Then
    '如果是字符串
    ...
    Else
      If YQasp.Debug Then
        YQasp.Error.Detail = n
        YQasp.Error.Raise "error-list-indexwrong"
      End If
    End If
  End Property
  Public Default Property Get At(ByVal n)
    If YQasp.Str.Test(n,"^\d+$") Then
      ...
      Else
        At = Null
        If YQasp.Debug Then
          YQasp.Error.Detail = Array(n, [End])
          YQasp.Error.Raise "error-list-outofrange"
        End If
      End If
    ElseIf YQasp.Str.Test(n,"^[\w-\./]+$") Then
      ...
      Else
        ...
        If YQasp.Debug Then
          YQasp.Error.Detail = Array(n, [End])
          YQasp.Error.Raise "error-list-outofhash"
        End If
      End If
    End If
  End Property
  ...
  Public Sub Data__(ByVal a, ByVal t)
    ...
    If isArray(a) Then
      ...
      For i = 0 To Ubound(a)
        If Instr(a(i),":")>0 Then
          j = YQasp.Str.GetName(a(i),":")
          If Not o_map.Exists(j) Then
            'YQasp.WN j
            o_map.Add i, j
            o_map.Add j, i
          End If
          a(i) = YQasp.Str.GetValue(a(i),":")
        End If
      Next
      a_list.SetArray a
    Else
			arr = YQasp.IIF(YQasp.IsN(s_separator), Split(a), Split(a, s_separator))
			a_list.SetArray arr
			Size = a_list.Length
			If t = 0 Then Exit Sub
			'如果有Hash特征值
			If Instr(a, ":")>0 Then
				For i = 0 To Ubound(arr)
					...
					If Instr(arr(i),":")>0 Then
						j = YQasp.Str.GetName(arr(i),":")
						If Not o_map.Exists(j) Then
							o_map.Add i, j
							o_map.Add j, i
						End If
						arr(i) = YQasp.Str.GetValue(arr(i),":")
					End If
				Next
			  a_list.SetArray arr
			End If
  ...
  Public Function Index(ByVal i)
    If isNumeric(i) Then
      Index = YQasp.IIF(i >= 0 And i <= [End], i, -1)
    Else
      If o_map.Exists(i) Then
        Index = o_map(i)
      Else
  ...
  Public Function IndexHash(ByVal i)
    If isNumeric(i) Then
      IndexHash = YQasp.IfThen(o_map.Exists(i), o_map(i))
    Else
      IndexHash = YQasp.IfThen(o_map.Exists(i), i)
    End If
    IndexHash = YQasp.IfHas(IndexHash, i)
  End Function
  ...
  Private Sub RemoveMap__(ByVal i)
    If o_map.Exists(i) Then
      ...
      'YQasp.WN "=Delete==mapRemove:" & o_map(i)
      o_map.Remove i
      'YQasp.WN "=Delete==mapRemove:" & i
    End If
  End Sub
  Private Sub UpFrom__(ByVal n, ByVal i)
    ...
    If o_map.Exists(i) Then
      ...
      o_map(n) = o_map(i)
      o_map.Remove i
      'YQasp.WN "=Delete==UpFromRemove:" & i & "  o_map(count):" & o_map.count
    End If
    At(n) = At(i)
  End Sub
    ...
    '如果插入到数组中间
      ...
      '将原数组插入点之后的值移动到新位置（腾出位置）
        ...
        '如果是数组，要腾出数组的长度个位置
          ...
          'YQasp.WN "把 " &i-1& "的值修改到 " &i+UBound(v)& " 上"
        Else
        '否则只腾出一个位置
          UpFrom__ i, i-1
        End If
  ...
  Public Sub [Delete](ByVal n)
    ...
    '如果是删除多个元素
      ...
      '单独处理每项
        ...
        If Instr(a(i),"-")>0 Then
          '取首元素
          x = Trim(YQasp.Str.GetName(a(i),"-"))
          '取尾元素
          y = Trim(YQasp.Str.GetValue(a(i),"-"))
          '如果是Hash则取出为数字下标
      ...
      For i = 0 To Ubound(a)
        If Instr(a(i),"-")>0 Then
          x = YQasp.Str.GetName(a(i),"-")
          y = YQasp.Str.GetValue(a(i),"-")
          'YQasp.WN a(i)
          tmp = tmp & x-1 & ","
          tmp = tmp & y+1 & "-"
        Else
  ...
  Public Sub Uniq()
    ...
    For i = 0 To [End]
      ...
      If indexOf__(arr, At(i)) = -1 Then
        ...
        arr(j) = At(i)
        'YQasp.WN "把元素" & i & "存入了新数组的 " & j
        If o_map.Exists(i) Then
          o_hash.Add j, o_map(i)
          o_hash.Add o_map(i), j
          'YQasp.WN "把Hash中的第("&i&")项" &o_map(i)& "存入新Hash的" & j
        End If
        j = j + 1
      End If
    Next
  ...
  Public Sub Rand
    ...
    For i = 0 To [End]
      j = YQasp.Str.RandomNumber(0,[End])
  ...
  Private Function SortArray(ByRef arr, ByRef low, ByRef high)
    ...
    If YQasp.IsN(arr) Then Exit Function
    Dim l, h, m, v, x
    l = low : h = high
  ...
  Private Sub AddHash__(ByVal arr)
    ...
    For i = 0 To Ubound(arr)
      '如果结果中有Hash下标
      'YQasp.WN arr(i) & " index:" & IndexOfHash(arr(i))
    ...
    For i = 0 To Ubound(a)
      ...
      If Instr(a(i),"-")>0 Then
        ...
        For j = x To y
          ...
          'YQasp.WN "Small:"&k & "=" & x & "-" & y
          arr(k) = At(j)
          If o_map.Exists(j) Then
            'YQasp.WN o_map(j) & " "&k&" " & o_hash.Exists(o_map(j))
      ...
      Else
        ...
        If o_map.Exists(x) Then
          '如果出现多个相同的Hash下标则只保留第一个Hash下标
          'YQasp.WN o_map(x) & " " & o_hash.Exists(o_map(x))
  ...
  Public Function Find(ByVal f)
    ...
    '默认标识符为 i
    k = "i"
    If YQasp.Str.Test(f,"[a-zA-Z]+:(.+)") Then
      '如果有自定义的标识符
      k = YQasp.Str.GetName(f,":")
      f = YQasp.Str.GetValue(f,":")
    End If
    k = "%" & k
    For i = 0 To [End]
  ...
  Private Function IsList(ByVal o)
    IsList = YQasp.Str.IsSame(TypeName(o), "YQAsp_list")
  End Function
  ...
  Public Sub Splice(ByVal o)
    If Not isArray(o) And Not isList(o) Then
      If YQasp.Debug Then
        YQasp.Error.FunctionName = "List.Splice"
        YQasp.Error.Raise "error-list-notlist"
      End If
      Exit Sub
    End If
    Dim omap,dic,i
    ...
    ElseIf IsList(o) Then
      ...
      If omap.Count > 0 Then
        For i = 0 To o.End
          'YQasp.WN YQasp.Str("{3}...{1} = {2}", Array(omap.Exists(i), (Not o_map.Exists(omap(i))), i))
          '取出Hash值名，存入原数组
          If omap.Exists(i) And (Not o_map.Exists(omap(i))) Then
            o_map.Add Size + i, omap(i)
            o_map.Add omap(i), Size + i
            'YQasp.WN YQasp.Str("{1} = {2}", Array(omap(i), Size + i))
          End If
        Next
      End If
      '把新值插入原数组
  ...
  Public Sub Inter(ByVal o)
    If Not isArray(o) And Not isList(o) Then
      If YQasp.Debug Then
        YQasp.Error.FunctionName = "List.Inter"
        YQasp.Error.Raise "error-list-notlist"
      End If
      Exit Sub
    End If
  ...
  Public Sub Diff(ByVal o)
    If Not isArray(o) And Not isList(o) Then
      If YQasp.Debug Then
        YQasp.Error.FunctionName = "List.Diff"
        YQasp.Error.Raise "error-list-notlist"
      End If
      Exit Sub
    End If
  ...
  Public Function Eq(ByVal o)
    If Not isArray(o) And Not isList(o) Then
      If YQasp.Debug Then
        YQasp.Error.FunctionName = "List.Eq"
        YQasp.Error.Raise "error-list-notlist"
      End If
      Exit Function
    End If
  ...
  Public Function Son(ByVal o)
    If Not isArray(o) And Not isList(o) Then
      If YQasp.Debug Then
        YQasp.Error.FunctionName = "List.Son"
        YQasp.Error.Raise "error-list-notlist"
      End If
      Exit Function
    End If
