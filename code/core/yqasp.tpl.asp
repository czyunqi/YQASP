...
'## YQasp.tpl.asp
'## -------------------------------------------------------------------
'## Feature     :   YQAsp Templates Class
'## Version     :   1.0
'## Author      :   云奇(114066164@qq.com)
'## Update Date :   2021-7-15
'## Description :   Use Templates with YQAsp
'##
'######################################################################
Class YQAsp_Tpl
  ...
  Private Sub LoadToTag(ByVal tag, ByVal t, ByVal f)
    ...
    Else
      s = f
    End If
    If YQasp.Has(tag) Then
      s_html = YQasp.Str.Replace(s_html, s_ms & tag & s_me, s)
    Else
      s_html = s_html & s
    End If
    SetBlocks()
  ...
  Public Sub [Update](ByVal b)
    ...
    End If
    Set Matches = YQasp.Str.Match(s_html, Chr(0) & b & Chr(0))
    Set Match = Matches
    For Each Match In Matches
      s = BlockTag(b)
  ...
  Private Function LogicReplace(ByVal s)
    ...
    Set Matches = YQasp.Str.Match(s, s_ms & "#if\s+(.+?)"&s_me&"([\s\S]+?)(?:"&s_ms&"#else"&s_me&"([\s\S]+?))?"&s_ms&"/#if"&s_me)
    For Each Match In Matches
      condi = Match.SubMatches(0)
      yes = Match.SubMatches(1)
      no = Match.SubMatches(2)
      '选择条件表达式组
      Set conds = YQasp.Str.Match(condi,"(?:([^&)(\s}|}=<>!]+)([=<>!]{1,2})(['""])(.+?)\3)|(?:([^&()\s|}=<>!]+)([=<>!]{1,2})([^&)\s|}]+))")
      For Each cond In conds
        '选择到的表达式
        cname = cond.Value
        '找其中的变量标签
        Set e = YQasp.Str.Match(cname,"@([\w\.]+)")
        For Each f In e
          n = f.SubMatches(0)
          '把标签替换为值
          cname = Replace(cname, f.Value, YQasp.IIF(o_tag.Exists(n),o_tag.Item(n),""))
        Next
        Set e = Nothing
        '解析表达式
        Set x = YQasp.Str.Match(cname,"^([^=<>!]*)([=<>!]{1,2})(['""]?)(.*)\3$")
        ckey = x(0).SubMatches(0)
        copera = x(0).SubMatches(1)
        cvalue = x(0).SubMatches(3)
        'YQasp.WNH "exp:" & ckey & copera & cvalue
        '比较表达式的结果
      ...
      'YQasp.WN yes
      'YQasp.WN no
      s = Replace(s, Match.Value, YQasp.IIF(Eval(condi), yes, no))
      Set conds = Nothing
    Next
    Set Matches = Nothing
    LogicReplace = s
  ...
  Private Function Comp(ByVal k, ByVal o, ByVal v)
    ...
    Select Case o
      Case "=","=="
        m = Replace(k,"\%","")
        If Instr(m,"%")>0 Then
          ma = YQasp.Str.GetName(m,"%")
          mb = YQasp.Str.GetValue(m,"%")
      ...
      Case "<" tmp = (CDbl(k) < CDbl(v))
    End Select
    Comp = YQasp.IIF(Err.Number=0,tmp,False)
  End Function
  '获取最终html
  Public Function GetHtml()
    ...
    '替换标签
    Set Matches = YQasp.Str.Match(s_html, s_ms & "([^:]+?)?(:.+?)?" & s_me)
    'YQasp.WN "rule:" & s_ms & "(.+?)" & s_me
    For Each Match In Matches
      n = Match.SubMatches(0)
      f = Match.SubMatches(1)
      f0 = YQasp.IIF(YQasp.Has(f),"{0"&f&"}","{0}")
      'YQasp.WT f0
      'YQasp.WN "match://" & Match.Value & "//"
      If o_tag.Exists(n) Then
        'YQasp.WT f0
        s_html = Replace(s_html, Match.Value, YQasp.Str.Format(f0,o_tag.Item(n)))
        's_html = Replace(s_html, Match.Value, o_tag.Item(n))
        'YQasp.WN "match_tag:" & Match.Value
        'YQasp.WN "match_dic:" & o_tag.Item(n)
      End If
    Next
    '替换未处理循环块
    Set Matches = YQasp.Str.Match(s_html, Chr(0) & "(\w+?)" & Chr(0))
    For Each Match In Matches
  ...
  Public Sub SaveAs(ByVal p)
    Call YQasp.Fso.CreateFile(p,GetHtml)
  End Sub
  '生成html标签
  Public Function MakeTag(ByVal t, ByVal f)
    Dim s,e,a,i,m
    If Instr(t,":")>0 Then
      m = YQasp.Str.GetValue(t,":")
      t = YQasp.Str.GetName(t,":")
      m = YQasp.Date.Format(Now,m)
    End If
    Select Case Lcase(t)
      Case "css"
        s = "<link href="""
    ...
    a = Split(f,"|")
    For i = 0 To Ubound(a)
      a(i) = s & Trim(a(i)) & YQasp.IfThen(YQasp.Has(m),"?" & m) & e
    Next
  ...
  Private Sub getMaskSE(ByVal m)
    s_ms = YQasp.Str.RegexpEncode(YQasp.Str.GetName(m,"*"))
    s_me = YQasp.Str.RegexpEncode(YQasp.Str.GetValue(m,"*"))
  End Sub
  '载入模板文件并将无限级include模板载入
  Private Function LoadInc(ByVal f, ByVal p)
    Dim h,pa,rule,inc,Match,incFile,incStr
    pa = YQasp.IIF(Left(f,1)="/","",p)
    If b_asp Then
      h = YQasp.GetInclude( pa & f )
    Else
      h = YQasp.Fso.Read( pa & f )
    End If
    rule = "(<!--[\s]*)?" & s_ms & "#include:(.+?)" & s_me & "([\s]*-->)?"
    If YQasp.Str.Test(h,rule) Then
      If YQasp.isN(p) Then
        If Instr(f,"/")>0 Then p = Left(f,InstrRev(f,"/"))
      Else
        If Instr(f,"/")>0 Then p = pa & Left(f,InstrRev(f,"/"))
      End If
      Set inc = YQasp.Str.Match(h,rule)
  ...
  Private Sub SetBlocks()
    ...
    i = 0
    rule = "(<!--[\s]*)?" & s_ms & "#:(.+?)" & s_me
    If Not YQasp.Str.Test(s_html, rule) Then Exit Sub
    Set Matches = YQasp.Str.Match(s_html,rule)
    '找到循环块
    For Each Match In Matches
  ...
  Private Sub Begin(ByVal b)
    ...
    '如果循环块有属性则取出属性
    If Instr(b," ")>0 Then
      attrs = YQasp.Str.GetValue(b, " ")
      b = YQasp.Str.GetName(b, " ")
      rule = "(<!--[\s]*)?(" & s_ms & ")#:(" & b & " " & YQasp.Str.RegexpEncode(attrs) & ")(" & s_me & ")([\s]*-->)?([\s\S]+?)(<!--[\s]*)?\2/#:" & b & "\4([\s]*-->)?"
    End If
    Set Matches = YQasp.Str.Match(s_html, rule)
    Set Match = Matches
    For Each Match In Matches
      'YQasp.WN "block_tag:" & b
      'YQasp.WN "block_attr:" & attrs
      '取循环块内容
      ...
      If YQasp.Has(attrs) Then
      '如果有属性则取出每个属性
        Set attr = YQasp.Str.Match(attrs,"((\w+)=(['""])(.+?)\3)|((\w+)=([^\s]+))")
        For Each att In attr
          aname = YQasp.Str.GetName(att.Value, "=")
          avalue = YQasp.Str.Replace(att.Value, "\w+=(['""]?)(.+?)\1", "$2")
          atag = b & "." & aname
          'YQasp.WN "attr '" & aname & "' = ["&avalue&"]"
          If o_attr.Exists(atag) Then o_attr.Remove(atag)
          o_attr.Add atag, avalue
        Next
      End If
      '把原始内容中的循环块作临时替换
      s_html = YQasp.Str.Replace(s_html, rule, Chr(0) & b & Chr(0))
    Next
    Set Matches = Nothing
  End Sub
  '取循环块原始模板数据
  ...
  Private Function UpdateBlockTag(ByVal s)
    ...
    Set Matches = YQasp.Str.Match(s, s_ms & "([^:]+?)?(:.+?)?" & s_me)
    For Each Match In Matches
      '取标签名
      data = Match.SubMatches(0)
      f = Match.SubMatches(1)
      ...
      If o_tag.Exists(data) Then
        rule = Match.Value      
        '替换标签为相应的值
        If YQasp.isN(o_tag.Item(data)) Then
          s = Replace(s, rule, "")
        Else
          s = Replace(s, rule, YQasp.Str.Format(f0,o_tag.Item(data)))
        End If
      End If
    Next
    UpdateBlockTag = s
