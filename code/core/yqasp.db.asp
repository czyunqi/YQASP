...
'## YQasp.db.asp
'## -------------------------------------------------------------------
'## Feature     :   YQAsp Database Control Class
'## Version     :   1.0
...
Class YQAsp_Db
  ...
  Private Sub Class_Initialize()
    YQasp.Error("error-db-conn") = YQasp.Lang("error-db-conn")
    YQasp.Error("error-db-noconn") = YQasp.Lang("error-db-noconn")
    YQasp.Error("error-db-execute") = YQasp.Lang("error-db-execute")
    YQasp.Error("error-db-batchselect") = YQasp.Lang("error-db-batchselect")
    YQasp.Error("error-db-executebatch") = YQasp.Lang("error-db-executebatch")
    YQasp.Error("error-db-paramarray") = YQasp.Lang("error-db-paramarray")
    YQasp.Error("error-db-select") = YQasp.Lang("error-db-select")
    YQasp.Error("error-db-getrecordset") = YQasp.Lang("error-db-getrecordset")
    YQasp.Error("error-db-insert") = YQasp.Lang("error-db-insert")
    YQasp.Error("error-db-insertbatch") = YQasp.Lang("error-db-insertbatch")
    YQasp.Error("error-db-delete") = YQasp.Lang("error-db-delete")
    YQasp.Error("error-db-deletebatch") = YQasp.Lang("error-db-deletebatch")
    YQasp.Error("error-db-update") = YQasp.Lang("error-db-update")
    YQasp.Error("error-db-updatebatch") = YQasp.Lang("error-db-updatebatch")
  ...
  Public Property Get PageOffset()
    PageOffset = YQasp.IIF(i_minRow = 0 , 0, i_minRow - 1)
  End Property
  '读取分页记录集总记录数
  ...
  Public Function OpenConnection(ByVal dbType, ByVal strDB, ByVal strServer)
    ...
    Select Case dbType
      Case "MSSQL"
        ConnStr = "Provider=sqloledb;Data Source=" & s & YQasp.IfThen(YQasp.Has(port), "," & port) & ";Initial Catalog="&strDB&";User Id="&u&";Password="&p&";"
      Case "ACCESS"
  ...
  Public Function CreateConnection(ByVal ConnStr)
    ...
    If Err.number <> 0 Then
      If Request.ServerVariables("LOCAL_ADDR") = Request.ServerVariables("REMOTE_ADDR") Then
        YQasp.Error.Detail = ", (""" & ConnStr & """)"
        YQasp.Error.FunctionName = "YQasp.Db.CreatConnection"
      End If
      YQasp.Error.Raise "error-db-conn"
    End If
  End Function
  ...
  Public Function GetConnection(ByVal connectionName)
    ...
    If o_connections.Exists(connectionName) Then
      ...
      If YQasp.Console.ShowSql Then YQasp.Console "连接到数据库(" & GetTypeVersion(GetConnection) & ")：" & Mid(o(1), InStrRev(o(1), "/")+1) & YQasp.IfThen(o(0)<>"ACCESS" ,"@" & Trim(Mid(o(2),InstrRev(o(2),"@")+1))) & "， 执行时间：" & YQasp.GetScriptTimeByTimer(t_start) & "s"
    Else
      '没有找到此名称的连接对象
      YQasp.Error.FunctionName = "YQasp.Db.GetConnection"
      YQasp.Error.Detail = connectionName
      YQasp.Error.Raise "error-db-noconn"
    End If
  End Function
  ...
  Public Property Let DefaultConn(ByRef conn)
    If TypeName(conn) = "Connection" Then
      If conn.State = 1 Then
        Set o_conn = conn
        If YQasp.Console.ShowSql Then YQasp.Console "接管外部连接为默认数据库(" & GetTypeVersion(conn) & ")"
      End If
    End If
  End Property
  ...
  Public Function GetType(ByRef conn)
    ...
    If YQasp.Str.IsSame(dbms, "MS Jet") Then
      s_type = "ACCESS"
    ElseIf YQasp.Str.IsSame(dbms, "Microsoft SQL Server") Then
      s_type = "MSSQL"
    ElseIf YQasp.Str.IsSame(dbms, "MySQL") Then
      s_type = "MYSQL"
    Else
      s_type = dbms
    End If
  ...
  Private Function ExecuteSql(ByRef conn, ByVal sql, ByVal executeType)
    ...
    sTimer = Timer()
    '判断是否是查询语句
    If YQasp.Str.IsSame(Left(sql, 7),"select ") Then queryType = 1
    '判断是否调用存储过程或函数
    If YQasp.Str.IsInList("call ,exec ", Left(sql, 5)) Then queryType = 4
    sql = ReplaceStasicParameter(sql)
    sql = ReplaceNewId(sql)
    outSql = sql
    o_sql = sql
    '查找参数标签
    Set match = YQasp.Str.Match(sql, "\{(.+?)\}")
    ...
    If matchCount>=0 Then
      ...
      For i = 0 To matchCount
        ...
        '取参数名
        param(i) = YQasp.Str.GetColonName(sqlParam)
        'YQasp.Println "p & i::" & param(i) & "---" & inOrOut(i)
        ...
        End If
        'YQasp.Println "p & i::" & param(i) & "---" & inOrOut(i)
        sqlParamType = YQasp.Str.GetColonValue(sqlParam)
        paramSize(i) = 8000
        '取数据类型，不设置默认为varchar类型
        If Instr(sqlParamType, "(") Then
          paramType(i) = GetParameterType(YQasp.Str.GetName(sqlParamType, "("))
          sqlParamSize = YQasp.Str.GetName(YQasp.Str.GetValue(sqlParamType, "("), ")")
          If IsNumeric(sqlParamSize) Then paramSize(i) = sqlParamSize
        Else
          paramType(i) = GetParameterType(sqlParamType)
        End If
        '取参数的原始值，并处理静态标签嵌套
        paramValue(i) = YQasp.Var(param(i))
        '替换输出SQL语句中的参数为值
    ...
    End If
    '在控制台中输出SQL语句
    If YQasp.Console.ShowSql Then YQasp.Console outSql
    sql = YQasp.Str.Replace(sql, "\{(.+?)\}", "?")
    '定义Command对象
    ...
    With cmd
      ...
      '如果是存储过程
        ...
        '解析参数
        spParam = Trim(YQasp.Str.GetValue(o_sql, spName))
        If YQasp.Has(spParam) Then
        '如果存储过程有参数
          spParams = Split(spParam, ",")
          'YQasp.PrintlnString spParams
          For i = 0 To UBound(spParams)
            ...
            If Left(spParam, 1) = "{" And Right(spParam, 1) = "}" Then
              ...
              spParam = YQasp.Str.GetColonName(spParam)
              'YQasp.Println spParam
              '取出参数的值和类型
              If matchCount>=0 Then
                For j = 0 To matchCount
                  If YQasp.Str.IsInList(param(j) & ",@" & param(j) & ",@@" & param(j), spParam) Then
                  '匹配到参数
            ...
            '输入参数
              ...
              If InStr(",20,11,6,5,7,135,3,131,4,2,16,128,205,204,", "," & spParamType & ",") > 0 And YQasp.IsN(spParamValue) Then spParamValue = Null
              ...
              'YQasp.PrintlnString Array(spParam, spParamType, spParamInorOut, spParamSize, spParamValue)
            Else
            '输出参数
              .Parameters.Append .CreateParameter(spParam, spParamType, 2, spParamSize)
              'YQasp.PrintlnString Array(spParam, spParamType, 2, spParamSize)
            End If
          Next
        End If
        ...
        rsState = sp.State
        'YQasp.Println "sp.State::" & sp.State
        'YQasp.Println "sp.RecordCount::" & sp.RecordCount
        '关闭了记录集才能取输出参数
        If rsState = 1 Then sp.Close
        Set List = YQasp.Json.NewObject
        '取输出参数
        Set outParams = YQasp.Json.NewObject
        ...
        If YQasp.Console.ShowSql And YQasp.Console.ShowSqlTime Then
          YQasp.Console "(" & YQasp.Lang("db-query-spend") & "：" & YQasp.GetScriptTimeByTimer(sTimer) & "s， " & YQasp.Lang("db-return") & "：" & returnValue & ")"
        End If
      Else
        If matchCount>=0 Then
        '如果sql中包含参数
          For i = 0 To matchCount
            '如果是数值型/日期型而且为空，则输入NULL值
            If InStr(",20,11,6,5,7,135,3,131,4,2,16,128,205,204,", ","&paramType(i)&",") > 0 And YQasp.IsN(paramValue(i)) Then paramValue(i) = Null
        ...
        '如果要返回记录集
          ...
          If YQasp.Console.ShowSql And YQasp.Console.ShowSqlTime Then
            ...
            YQasp.Console "(" & YQasp.Lang("db-query-spend") & "：" & YQasp.GetScriptTimeByTimer(sTimer) & "s， " & YQasp.Lang("db-record-count") & "：" & i_rcount & ")"
          End If
        ...
        '如果仅返回执行成功与否
          ...
          i_queryTimes = i_queryTimes + 1
          If YQasp.Console.ShowSql And YQasp.Console.ShowSqlTime Then
            YQasp.Console "(" & YQasp.Lang("db-query-spend") & "：" & YQasp.GetScriptTimeByTimer(sTimer) & "s， " & YQasp.IIF(ExecuteSql, YQasp.Lang("db-query-success"), YQasp.Lang("db-query-fail")) & "， " & YQasp.Lang("db-affected-rows") & "：" & affectedRows & ")"
          End If
        ...
        '返回受影响的行数
          ...
          i_queryTimes = i_queryTimes + 1
          If YQasp.Console.ShowSql And YQasp.Console.ShowSqlTime Then
            YQasp.Console "(" & YQasp.Lang("db-query-spend") & "：" & YQasp.GetScriptTimeByTimer(sTimer) & "s， " & YQasp.Lang("db-affected-rows") & "：" & affectedRows & ")"
          End If
        End If
      End If
    End With
  ...
  Private Sub CheckError(ByVal name, ByRef e, ByRef conn, ByRef funName, ByVal detail)
    ...
    If b_tmp Then
      If YQasp.Debug Then
        YQasp.Error.SetErrors e, conn, Null
        YQasp.Error.FunctionName = "YQasp.Db." & funName
        YQasp.Error.Detail = detail & YQasp.Lang("db-see-sql-in-console")
        YQasp.Error.Raise "error-db-" & name
      End If
    End If
  End Sub
  ...
  Public Function Execute(ByRef conn, ByVal sql)
    On Error Resume Next
    '判断是否是查询语句
    If YQasp.Str.IsSame(Left(sql, 7),"select ") Or YQasp.Str.IsInList("call ,exec ", Left(sql, 5)) Then
  ...
  Private Function GetBatchArray(ByVal sql)
    ...
    If matchCount>=0 Then
      ...
      For i = 0 To matchCount
        ...
        param(i) = YQasp.Str.GetColonName(sqlParam)
        '取数据类型
        paramType(i) = GetParameterType(YQasp.Str.GetColonValue(sqlParam))
        '如果参数是数组
        a_tmp = YQasp.Var(param(i)&"_array")
        If IsArray(a_tmp) Then
          If Ubound(a_tmp) > 0 Then
            ...
            If sqlCount > 0 And sqlCount <> a_tmplen Then
              YQasp.Error.FunctionName = "YQasp.Db.GetBatchArray"
              YQasp.Error.Detail = param(i)
              YQasp.Error.Raise "error-db-paramarray"
              Exit Function
            End If
            '将所有是数组的参数用一个数组保存起来
  ...
  Public Function GetRecordSet(ByRef conn, ByVal sql)
    ...
    sql = ReplaceStasicParameter(sql)
    If YQasp.Console.ShowSql Then
      'YQasp.Console "(分页) " & ToSql(sql)
      YQasp.Console "(== " & YQasp.Lang("db-pager-start") & " ==)"
    End If
    '取得数据库类型及版本
    ...
    b_getPK = s_dbType = "ACCESS" Or _
                     (s_dbType = "MSSQL" And _
                       (YQasp.Str.GetName(s_dbVer,".") < 11 Or _
                         Not YQasp.Str.IsIn(s_tmp, "order by")))
    If b_getPK Then
      '取主键名
      s_keySql = Replace(sql, "Select", "SELECT TOP 1", 1, 1, 1)
      s_keySql = "SELECT * FROM (" & s_keySql & ") AS YQAsp_Pager_Key_Table WHERE 1=0"
      Set rsTmp = ExecuteSql(conn, s_keySql, 1)
      'i_queryTimes = i_queryTimes - 1
      s_pkey = rsTmp.Fields(0).Name
      Close(rsTmp)
      'YQasp.Console s_pkey
    End If
    If YQasp.Str.IsIn(s_tmp, "order by") Then
    ...
    End If
    '取总记录数
    s_sqlCount = "SELECT COUNT(*) AS YQAsp_Record_Count FROM (" & s_sqlNoOrder & ") AS YQAsp_Pager_Count_Table"
    Set rsTmp = ExecuteSql(conn, s_sqlCount, 1)
    i_recordCount = Int(Trim(rsTmp(0).Value))
    Close(rsTmp)
    If i_recordCount > 0 Then
      If YQasp.Has(YQasp.Var(s_offsetParam)) Then
        i_offset = Int(YQasp.Var(s_offsetParam))
      End If
      If YQasp.Has(YQasp.Var(s_limitParam)) And b_limitEnable Then
        i_pageSize = Int(YQasp.Var(s_limitParam))
      End If
      i_tmp = i_recordCount / i_pageSize
      i_pageCount = Int(i_tmp) + YQasp.IIF(Int(i_tmp)=i_tmp, 0, 1)
      If YQasp.Has(YQasp.Var(s_offsetParam)) Then
        i_minRow = i_offset + 1
      ...
      Else
        If s_dbType = "MSSQL" And YQasp.Str.GetName(s_dbVer,".") >= 9 Then
          If YQasp.Str.GetName(s_dbVer,".") >= 11 Then
          ...
          Else
            ...
            s_sql = "SELECT * FROM (SELECT *,ROW_NUMBER() OVER (" & s_order & ") AS YQAsp_Pager_RowRank FROM (" & s_sqlNoOrder & ") AS YQAsp_Pager_Max_Table) AS YQAsp_Pager_Result_Table WHERE YQAsp_Pager_Result_Table.YQAsp_Pager_RowRank BETWEEN " & i_minRow & " AND " & i_maxRow
          End If
        Else
        '如果是Access或者SQL Server 2000及以下版本
          sql = Replace(sql, "Select", "SELECT TOP " & i_maxRow, 1, 1, 1)
          s_sql = "SELECT * From (" & sql & ") AS YQAsp_Pager_Result_Table WHERE " & s_pkey & " IN (SELECT TOP " & i_rsSize & " " & s_pkey & " FROM (" & sql & ") AS YQAsp_Pager_Max_Table " & ReverseOrderBy(s_order) & ") " & s_order & ""
        End If
      End If
    ...
    '输出执行时间及执行结果
    If YQasp.Console.ShowSql And YQasp.Console.ShowSqlTime Then
      s_tmp = "(== " & YQasp.Lang("db-pager-finish") & YQasp.Lang("db-query-spend") & "：" & YQasp.GetScriptTimeByTimer(t_start) & "s"
      s_tmp = s_tmp & YQasp.Str.Format(YQasp.Lang("db-pager-in-console"), Array(i_rsSize, i_minRow, i_maxRow, i_recordCount, i_pageSize, i_pageIndex, i_pageCount))
      s_tmp = s_tmp & " ==)"
      YQasp.Console s_tmp
    End If
  ...
  Private Function ReverseOrderBy(ByVal string)
    ...
    For i = 0 To UBound(a_fields)
      ...
      If YQasp.Str.IsSame(Right(s_tmp, 5), " desc") Then
        s_field = Left(s_tmp, Len(s_tmp) - 5)
        s_sort = " ASC"
      ElseIf YQasp.Str.IsSame(Right(s_tmp, 4), " asc") Then
  ...
  Private Function GetPageIndex()
    Dim i_page
    i_page = YQasp.Var(s_pageParam)
    i_page = YQasp.IIF(isNumeric(i_page) And YQasp.Has(i_page) And i_page > "0", i_page, 1)
  ...
  Public Function Pager(ByVal html, ByRef config)
    ...
    s_tmp = YQasp.IfHas(html, o_pager("default_html"))
    ...
    '读入配置信息
    '如果为空则读入默认配置
    config = YQasp.IfHas(config, o_pager("default_config"))
    '如果配置不是数组则转为数组
    If YQasp.Has(config) Then config = YQasp.IIF(IsArray(config), config, Array(config,"userconfig:1"))
    If isArray(config) Then
      Dim s_name, s_value
      For i = 0 To Ubound(config)
        s_name = YQasp.Str.GetColonName(config(i))
        s_value = YQasp.Str.GetColonValue(config(i))
        If YQasp.Str.IsInList("recordcount,pageindex,pagecount,pagesize,listlong,listsidelong,jumplong", s_name) Then
    ...
    If o_cfg("listsidelong")>0 Then
      ...
      If o_cfg("listsidelong") < i_start Then
        For i = 1 To o_cfg("listsidelong")
          ...
          End If
        Next
        s_moreTmp = YQasp.IfThen(o_cfg("listsidelong") + 1 <> i_start, o_cfg("more"))
        If o_cfg("listtype") = "ul" Then
    ...
    Select Case LCase(o_cfg("jump"))
      Case "input"
        ...
        s_jump = " <input type=""text"" size=""3"" title=""" & YQasp.Lang("db-pager-input-text") & """ " & YQasp.IfHas(o_cfg("jumpplus"),"")
        '回车键执行跳转
        s_jump = s_jump & " onkeydown=""javascript:if(event.charCode==13||event.keyCode==13){if(!isNaN(" & s_jumpValue & ")){"
        s_jump = s_jump & YQasp.IIF(o_cfg("jumpaction")="",YQasp.IIF(Lcase(Left(o_cfg("link"),11))="javascript:",Replace(Mid(o_cfg("link"),12),"*",s_jumpValue),"document.location.href='" & Replace(o_cfg("link"),"*","'+" & s_jumpValue & "+'") & "';"),Replace(o_cfg("jumpaction"),"*", s_jumpValue))
        s_jump = s_jump & "}return false;}"" /> "
      Case "select"
        '生成跳转下拉框
        s_jumpValue = "this.options[this.selectedIndex].value"
        s_jump = " <select " & YQasp.IfHas(o_cfg("jumpplus"),"") & " onchange=""javascript:"
        s_jump = s_jump & YQasp.IIF(o_cfg("jumpaction")="",YQasp.IIF(Lcase(Left(o_cfg("link"),11))="javascript:",Replace(Mid(o_cfg("link"),12),"*",s_jumpValue),"document.location.href='" & Replace(o_cfg("link"),"*","'+" & s_jumpValue & "+'") & "';"),Replace(o_cfg("jumpaction"),"*",s_jumpValue))
        s_jump = s_jump & """ title=""" & YQasp.Lang("db-pager-select-text") & """> "
        '下拉框下拉菜单数量
        If o_cfg("jumplong")=0 Then
          For i = 1 To o_cfg("pagecount")
            s_jump = s_jump & " <option value=""" & i & """" & YQasp.IfThen(i=o_cfg("pageindex")," selected=""selected""") & ">" & i & "</option> "
          Next
        Else
          i_jumpLong = Int(o_cfg("jumplong") / 2)
          i_jumpStart = YQasp.IIF(o_cfg("pageindex")-i_jumpLong<1, 1, o_cfg("pageindex")-i_jumpLong)
          i_jumpStart = YQasp.IIF(o_cfg("pagecount")-o_cfg("pageindex")<i_jumpLong, i_jumpStart-(i_jumpLong-(o_cfg("pagecount")-o_cfg("pageindex")))+1, i_jumpStart)
          i_jumpStart = YQasp.IIF(i_jumpStart<1,1,i_jumpStart)
          j = 1
          For i = i_jumpStart To o_cfg("pageindex")
            s_jump = s_jump & " <option value=""" & i & """" & YQasp.IfThen(i=o_cfg("pageindex")," selected=""selected""") & ">" & i & "</option> "
            j = j + 1
          Next
          i_jumpLong = YQasp.IIF(o_cfg("pagecount")-o_cfg("pageindex")<i_jumpLong, i_jumpLong, i_jumpLong + (i_jumpLong-j)+1)
          i_jumpEnd = YQasp.IIF(o_cfg("pageindex")+i_jumpLong>o_cfg("pagecount"), o_cfg("pagecount"), o_cfg("pageindex")+i_jumpLong)
  ...
  Private Function AddHtmlClass(ByVal hasClass, ByVal nameArray)
    Dim i, s_tmp, b_tmp
    If YQasp.IsN(nameArray) Then AddHtmlClass = "" : Exit Function
    If Not IsArray(nameArray) Then nameArray = Array(nameArray)
    For i = 0 To UBound(nameArray)
      If YQasp.Has(nameArray(i)) Then
        If b_tmp Or hasClass = 1 Then s_tmp = s_tmp & " "
        s_tmp = s_tmp & nameArray(i)
        b_tmp = True
      End If
    Next
    If hasClass = 0 And YQasp.Has(s_tmp) Then s_tmp = " class=""" & s_tmp & """"
    AddHtmlClass = s_tmp
  End Function
  ...
  Private Function ReplaceUrl(ByVal param, ByVal value)
    Dim a_rwt, o_matches
    a_rwt = YQasp.IsRewriteRule()
    If a_rwt(0) Then
      '如果是伪静态页面
      Set o_matches = YQasp.Str.Match(a_rwt(2), param & "=(\$\d)")
      If o_matches.Count > 0 Then
        ReplaceUrl = YQasp.Str.ReplacePart(a_rwt(3), a_rwt(1), o_matches(0).SubMatches(0), value)
      Else
        ReplaceUrl = a_rwt(3)
      End If
      Set o_matches = Nothing
    Else
      ReplaceUrl = YQasp.GetUrlWith("-" & s_offsetParam & ",-" & param, param & "=" & value)
    End If
  End Function
  ...
  Public Sub SetPager(ByVal pagerName, ByVal html, ByRef config)
    pagerName = YQasp.IfHas(pagerName, "default")
    If YQasp.Has(html) Then o_pager(pagerName & "_html") = html
    If YQasp.Has(config) Then o_pager(pagerName & "_config") = config
  End Sub
  ...
  Private Function GetFiledValues(ByVal fieldValues, ByRef conn)
    ...
    '如果没有字段只有值
    hasFields = InStr(YQasp.Str.Replace(fieldValues, "\{(.+?)\}", "?"), ":") > 0
    If hasFields Then ReDim fields(i_fieldValuesLength)
    ReDim values(i_fieldValuesLength)
    '取出字段名称和值参数
    For i = 0 To i_fieldValuesLength
      If hasFields Then
        fields(i) = FixName(Trim(YQasp.Str.GetColonName(a_fieldValues(i))), conn)
        values(i) = Trim(YQasp.Str.GetColonValue(a_fieldValues(i)))
      Else
        '如果没有字段名
  ...
  Private Function InsertRecord(ByRef conn, ByVal table, ByVal fieldValues, ByVal IsBatch)
    ...
    If IsBatch Then
      ...
      'MSSQL数据库
      If (s_dbType = "MSSQL" And YQasp.Str.GetName(s_dbVer,".") >= 9) Or s_dbType = "MYSQL" Then
        Select Case s_dbType
          Case "MSSQL"
            'MSSQL参数不能超过2100个，同时插入不能超过1000条
            i_paramlimit = 2000
            i_sqllimit = 1000
            i_sqllimit = YQasp.IIF(Int(i_paramlimit/i_paramCount)>i_sqllimit,i_sqllimit,Int(i_paramlimit/i_paramCount))
          Case "MYSQL"
            'MySQL数据库限制为同时最多插入5000条，避免存储空间不足错误
            i_sqllimit = 5000
        End Select
        i_limit = i_valuesCount/i_sqllimit
        i_limit = Int(i_limit) + YQasp.IIF(i_limit>Int(i_limit),1,0) - 1
        For i1 = 0 To i_limit
          i2 = i1*i_sqllimit
          ...
          Next
          '执行批量插入
          'YQasp.console i_limit & " / " & i1 & " / " & i2 & " / " & i3 & " / " & i4
  ...
  Public Function Delete(ByRef conn, ByVal table, ByVal where)
    ...
    If YQasp.Has(where) Then sql = sql & " Where " & where
  ...
  Private Function FixName(ByVal string, ByRef conn)
    ...
    Select Case UCase(GetType(conn))
      Case "ACCESS", "MSSQL"
        If Not YQasp.Str.Test(string, "^(\[.+\]|"".+"")$") Then
          string = "[" & string & "]"
        End If
      Case "MYSQL"
        If Not YQasp.Str.Test(string, "^`.+`$") Then
    ...
    FixName = string
  End Function
  '替换SQL语句中的{YQasp.newid}
  Private Function ReplaceNewId(ByVal sql)
    If InStr(1,sql, "{YQasp.newid}",1)>0 Then
      ReplaceNewId = Replace(sql, "{YQasp.newid}", "'" & YQasp.NewID() & "'", 1, -1, 1)
    Else
      ReplaceNewId = sql
    End If
  End Function

  '替换SQL语句中的静态变量 {=YQasp变量名}
  Private Function ReplaceStasicParameter(ByVal sql)
    Dim matches, match, mparam, mvar, mtype, marr, i, tmp
    If Instr(sql, "{=") Then
      Set matches = YQasp.Str.Match(sql, "\{=(.+?)\}")
      For Each match In matches
        'YQasp.Console match
        sql = Replace(sql, match, YQasp.Var(match.SubMatches(0)), 1, -1, 1)
      Next
      Set matches = Nothing
    End If
    If Instr(sql, "{(") Then
      Set matches = YQasp.Str.Match(sql, "\{\((.+?)\)\}")
      For Each match In matches
        'YQasp.Console Match.value
        tmp = ""
        mparam = match.SubMatches(0)
        mvar = YQasp.Str.GetColonName(mparam)
        mtype = YQasp.Str.GetColonValue(mparam)
        If YQasp.Has(mtype) Then mtype = ":" & mtype
        marr = YQasp.Var(mvar & "_array")
        'YQasp.Console marr
  ...
  Private Function GetParameter(ByVal param)
    ...
    arr(0) = YQasp.Str.GetColonName(param)
    arr(1) = YQasp.Str.GetColonValue(param)
    arr(2) = YQasp.Var(arr(0))
    GetParameter = arr
  End Function
  ...
  Private Function FormatValue(ByVal value, ByVal ptype)
    ...
    Select Case ptype
      ...
      Case 7,133,134,135
        ...
        End If
      Case 13, 128, 204, 205
        s_tmp = YQasp.IIF(YQasp.Has(value),"(blob)","NULL")
      Case Else
        s_tmp = YQasp.IfHas(value,"NULL")
        s_tmp = Replace(s_tmp, "'", "''")
    End Select
    FormatValue = s_tmp
  End Function
  ...
  Public Function BeginTrans(ByRef conn)
    ...
    If YQasp.Console.ShowSql Then YQasp.Console YQasp.Str.Format(YQasp.Lang("db-trans-start"), BeginTrans)
  End Function
  '回滚一个事务
  Public Sub RollbackTrans(ByRef conn)
    conn.RollbackTrans
    If YQasp.Console.ShowSql Then YQasp.Console YQasp.Lang("db-trans-rollback")
  End Sub
  '提交一个事务
  Public Sub CommitTrans(ByRef conn)
    conn.CommitTrans
    If YQasp.Console.ShowSql Then YQasp.Console YQasp.Lang("db-trans-commit")
  End Sub
  '开始默认连接的事务
  Public Sub Begin()
    OpenConn()
