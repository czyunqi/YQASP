...
Class YQAsp_Log
  ...
  Private Sub init()
    If Not b_inited Then
      ...
      s_id = ""
      s_path = "/../"
      Set dic_style = YQasp.Json.NewObject
      ...
      s_siteFolder = YQasp.Str.GetValueRev(YQasp.Fso.MapPath("/"), "\")
      s_logFolder = s_siteFolder & "_log/"
      s_rolling = "day"
      s_file = makeFileName("ymmdd")
      Set dic = YQasp.Json.NewObject()
      dic("ip") = YQasp.GetIp()
      dic("url") = YQasp.GetUrl("")
  ...
  Private Function makeFileName(Byval fileStyle)
    makeFileName = Year(Now) & "_" & Right("0" & Month(Now), 2) & "/" & s_siteFolder & _
                   YQasp.IfThen(YQasp.Has(s_id), "_" & s_id) & _
                   "_" & YQasp.Date.Format(Now(), fileStyle)
  End Function
  ...
  Public Property Let Appender(ByVal string)
    ...
    For i = 0 To Ubound(t)
      ti = Trim(t(i))
      If YQasp.Str.IsSame(ti, "file") Then
        b_file = True
      ElseIf YQasp.Str.IsSame(ti, "db") Then
        b_db = True
      End If
    Next
  End Property
  ...
  Public Property Get Appender
    init()
    Dim a : Set a = YQasp.Json.NewArray()
  ...
  Public Property Get SavePath
    init()
    SavePath = YQasp.Fso.MapPath(s_path) & "\" & s_siteFolder
  End Property
  ...
  Private Sub makeFile(ByVal t, ByVal msg, ByVal source)
    ...
    If YQasp.Has(source) Then
      line = YQasp.Str.GetValueRev(source, ":")
      If IsNumeric(line) Then
        dic_add("fn") = YQasp.Str.GetNameRev(source, ":") & ", line " & line
      Else
        dic_add("fn") = source
      End If
    End If
    txt = YQasp.Str.Format(dic_style(t), dic_add)
    Set dic_add = Nothing
    fileName = s_path & s_logFolder & s_file & "_" & Lcase(t) & ".log"
    If YQasp.Fso.IsFile(fileName) Then
      Call YQasp.Fso.AppendFile(fileName, txt & vbCrLf)
    Else
      header = "# " & t & " logs for " & s_siteFolder & vbCrLf
      Call YQasp.Fso.CreateFile(fileName, header & txt & vbCrLf)
    End If
  End Sub
