...
'## Description :   YQAsp main class
'##
'######################################################################
Server.ScriptTimeOut = 300
Response.Buffer = True  '设置输出缓冲区
Dim YQasp_Include_html
Dim YQasp_Timer : YQasp_Timer = Timer() '设置计时器
Dim YQasp : Set YQasp = New YQAsp '实例化YQasp
%>
<!--#include file="yqasp.config.asp" -->
<%
Class YQAsp
  '定义YQasp公共类
  ...
  Private Sub Class_Terminate()
    ...
    Core_Do "off", s_cores
    Set Lang = Nothing
    'YQasp.Println YQasp.GetScriptTimeByTimer(YQasp_Timer)
  End Sub
    ...
    Select Case t
      Case "on"
        For i = 0 To Ubound(a_core)
          Execute "Set " & a_core(i) & " = New YQAsp_object"
        Next
      Case "off"
  ...
  '终止程序运行
  Public Sub [Exit]():  Set YQasp = Nothing : Response.End() : End Sub
  ...
  Public Function GetIncCode(ByRef content, ByRef getHtml)
    ...
    s_codeBegin = IIF(getHtml, "YQasp_Include_html = YQasp_Include_html & ", "Response.Write ")
    Do While i_endPos > i_startPos + 1
      ...
      If getHtml Then
        ...
        s_codeTmp = Str.ReplaceLine(s_codeTmp, "^(\s*)YQasp\.(Echo|Print|Println)([\( ])", "$1" & s_codeBegin & "$3") & vbCrLf
      End If
    ...
    If Has(s_tmp) Then
      ...
      s_code = s_code & s_codeBegin & """" & s_tmp & """" & vbCrLf
    End If
    If getHtml Then s_code = "YQasp_Include_html = """" " & vbCrLf & s_code
  ...
  Public Default Function Ext(ByVal name)
    ...
    If Not o_ext.Exists(name) Then
      b_loaded = False
    Else
      If LCase(TypeName(o_ext(name))) <> "YQAsp_" & name Then b_loaded = False
    End If
    If Not b_loaded Then
      s_filePath = s_pluginPath & "YQasp." & name & ".asp"
      If Fso.isFile(s_filePath) Then
        Include s_filePath
        Execute("Set o_ext(""" & name & """) = New YQAsp_" & name)
      Else
        If YQasp.Debug Then
          '如果插件不存在则抛出异常
          [Error].FunctionName = "YQasp.Ext(""" & name & """)"
          [Error].Detail = s_filePath
          [Error].Raise "error-YQasp-pluginpath"
        End If
      End If
    End If
    Set Ext = o_ext(name)
  ...
  Public Function GetVal(ByVal string)
    Set GetVal = New YQAsp_Validation
    GetVal.Source = YQasp.Get(string)
  End Function
  Public Function PostVal(ByVal string)
    Set PostVal = New YQAsp_Validation
    PostVal.Source = YQasp.Post(string)
  End Function
  Public Function VarVal(ByVal string)
    Set VarVal = New YQAsp_Validation
    VarVal.Source = YQasp.Var(string)
  End Function
  ...
  Public Sub Init() '初始化YQAsp
    Set [Error] = New YQAsp_Error
    [Error]("error-YQasp-pluginpath") = Lang("error-YQasp-pluginpath")
    Set Fso     = New YQAsp_Fso
    Set Str     = New YQAsp_String
    Set Console = New YQAsp_Console
    Set Var     = New YQAsp_Var
    Set [Date]  = New YQAsp_Date
    Set Db      = New YQAsp_Db
    Set Encrypt = New YQAsp_Encrypt
    Set Json    = New YQAsp_Json
    Set List    = New YQAsp_List
    Set Upload  = New YQAsp_MoLibUpload
    Set Http    = New YQAsp_Http
    Set Tpl     = New YQAsp_Tpl
    Set Cache   = New YQAsp_Cache
    Set Xml     = New YQAsp_Xml
    Set Tar     = New YQAsp_Tar
    Set Log     = New YQAsp_Log
  End Sub

End Class
Class YQAsp_object : End Class
%>
<!--#include file="core/yqasp.stringbuilder.asp"-->
<!--#include file="core/yqasp.error.asp"-->
<!--#include file="core/yqasp.validation.asp"-->
<!--#include file="core/yqasp.str.asp"-->
<!--#include file="core/yqasp.stringobject.asp"-->
<!--#include file="core/yqasp.var.asp"-->
<!--#include file="core/yqasp.console.asp"-->
<!--#include file="core/yqasp.date.asp"-->
<!--#include file="core/yqasp.db.asp"-->
<!--#include file="core/yqasp.encrypt.asp"-->
<!--#include file="core/yqasp.json.asp"-->
<!--#include file="core/yqasp.list.asp"-->
<!--#include file="core/yqasp.fso.asp"-->
<!--#include file="core/yqasp.upload.asp"-->
<!--#include file="core/yqasp.http.asp"-->
<!--#include file="core/yqasp.tpl.asp"-->
<!--#include file="core/yqasp.cache.asp"-->
<!--#include file="core/yqasp.xml.asp"-->
<!--#include file="core/yqasp.tar.asp"-->
<!--#include file="core/yqasp.log.asp"-->