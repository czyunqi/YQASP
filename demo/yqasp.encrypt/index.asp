...
If YQasp.Get("action") = "en" Then
  i_type = YQasp.Post("type")
  YQasp.Encrypt.Key = YQasp.Post("key")
  If i_type = 0 Then
    s_source = YQasp.Post("source")
    s_target = YQasp.Encrypt(s_source)
    YQasp.PrintEnd s_target
  ElseIf i_type = 1 Then
    s_target = YQasp.Post("target")
    s_source = YQasp.Encrypt.Decrypt(s_target)
    YQasp.PrintEnd s_source
  End If
End If
%>
<!DOCTYPE html>
...
<body>
	<div class="container">
  	<div>
    	<textarea name="source" id="source">YunqASP 加密解密类 YQasp.Encrypt 采用了一种高效率的对称加密算法，用于普通的加解密场景应该是足够了，密钥设置得越长安全性越高，优点是速度非常快。
使用方法
&lt;%
'设置密钥
YQasp.Encrypt.Key = "encrypt_key"
'加密
YQasp.Encrypt(string)
'解密
YQasp.Encrypt.Decrypt(string)
'用指定密钥加密
YQasp.Encrypt.EncryptBy(string, "encrypt_key")
'用指定密钥解密
YQasp.Encrypt.DecryptBy(string, "encrypt_key")
%&gt;
Unicode字符示例：
