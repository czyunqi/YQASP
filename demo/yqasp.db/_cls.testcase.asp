...
'设置YQasp数据源：
' 设置默认的数据源
YQasp.Db.SetConnection "default", 1, "_sample.mdb", ""
' 或者
'YQasp.Db.SetConn "ACCESS", "_sample.mdb", ""


Dim TestCase
Set TestCase = New YunqASP_TestCase

'测试类
Class YunqASP_TestCase
  Public Db
  ...
  Public Function ShowScriptTime()
    YQasp.print "<small style=""color:#ccc"">页面执行时间 " & YQasp.GetScriptTime() & " 秒, 数据库查询 " & YQasp.Db.QueryTimes & " 次</small>"
  End Function
End Class
...
Class EasyAsp_TestCase_Db
  ...
  Sub ShowRecordSet(ByVal rs, ByVal num)
    Dim s_tmp, field, i, n
    n = YQasp.IIF(IsNumeric(num),num,0)
    s_tmp = "<style>.yqasp-test-table{width:100%;margin:0;padding:0;border-collapse:collapse;border:1px solid #555;border-bottom:none;}.yqasp-test-table th{background-color:#EEE;white-space:nowrap;}.yqasp-test-table thead th{background-color:#CCC;}.yqasp-test-table th,.yqasp-test-table td{font-size:12px;border:1px solid #999;padding:4px;word-break:break-all;}</style>"
    s_tmp = s_tmp & "<table class=""yqasp-test-table"">"
    If YQasp.Has(rs) Then
      ...
      For i = 0 To n-1
        ...
        s_tmp = s_tmp & "<tr>"
        For Each field In rs.Fields
          s_tmp = s_tmp & "<td>" & YQasp.IIF(TypeName(field.Value)="Byte()", "[ 二进制文件 ]", field.Value) & "</td>"
        Next
        s_tmp = s_tmp & "</tr>"
        rs.MoveNext
      Next
  ...
  Private Function CreateSelectOption(ByVal defaultValue, ByVal rs, ByVal parentValue, ByVal strSplit, ByVal strTab)
    ...
    While Not rs_tmp.Eof
      ...
      '如果是默认值添加selected属性
      s_tmp = s_tmp & YQasp.IfThen(YQasp.Str.IsSame(defaultValue, rs_tmp(0)), " selected")
      ...
      Else
        ...
        s_tmp = s_tmp & ">" & s_split & YQasp.IIF(isLast,"┖ ","┠ ") & rs_tmp(1) & "</option>"
        '如果是本级最后一条记录，则将隐藏左侧前导符竖线标志传到下级处理
        strTab =  YQasp.IIF(isLast, Left(strTab,Len(strTab)-1) & "┊", strTab)
      End If
