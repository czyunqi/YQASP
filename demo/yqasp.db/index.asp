...
<%
Dim result, act
act = LCase(YQasp.Get("action"))
Select Case act
  Case "savenew", "saveedit"  '添加或编辑内容
    '在这里编写你的服务端表单验证代码
    Call YQasp.PostVal("title").Name("标题").Required.Alert
    Call YQasp.PostVal("classid").Name("所属类别").Required.Maxlength(10).Test("^\w+$").Msg("%n不是正确的ID序号").Alert
    YQasp.Var("post.announcetime") = YQasp.VarVal("post.announcetime").Name("发布时间").Default(Now()).IsDate.Alert
    YQasp.PostVal("content").Name("内容").Required.Alert
    '此处仅作简单的空值判断
    'If YQasp.IsN(YQasp.Var("post.title")) Or YQasp.IsN(YQasp.Var("post.classid")) Or YQasp.IsN(YQasp.Var("post.announcetime")) Or YQasp.IsN(YQasp.Var("post.content")) Then
    '  YQasp.RR "?info=" & act & "error"
    'End If
    If act = "savenew" Then
      '添加到数据库
      result = YQasp.Db.Ins("EC_Content", "ContentID:{yqasp.newid}, ContentClassID:{post.classid}, ContentTitle:{title}, AnnounceTime:{announcetime},ContentText:{content}")
    Else
      result = YQasp.Db.Upd("EC_Content", "ContentClassID={post.classid}, ContentTitle={title}, AnnounceTime={announcetime},ContentText={content}", "ContentID={cid}")
    End If
    If result Then YQasp.RR "?info=" & act & "success"
  Case "delete"
    '判断有没有选择内容
    If YQasp.IsN(YQasp.Var("id")) Then YQasp.RR "?info=deleteempty"
    '批量删除（post和get方法提交的数据均可）
    result = YQasp.Db.DelBatch("EC_Content","ContentID = {id}")
    YQasp.Console "delete " & result & " records."
    YQasp.RR "?info=deletesuccess"
End Select
...
Function ShowAlert()
  ...
  Select Case YQasp.Var("info")
    ...
    Case Else
      ...
      Exit Function
  End Select
  ShowAlert = YQasp.Str.Format(str,arr)
End Function
%>
<!DOCTYPE html>
<html lang="en">
  <head>
    ...
    <title>Access数据库增删改查示例 - YunqASP测试用例中心</title>
    <!-- Bootstrap -->
...
YQasp.Var("likeKey") = "%{=key}%"
'获取内容记录集
Set rs = YQasp.Db.GetRS("Select a.ContentID, a.ContentClassID, b.ContentClassName, a.ContentTitle, a.AnnounceTime, b.SortLevel From EC_Content a Inner join EC_ContentClass b On a.ContentClassID = b.ContentClassID Where a.AnnounceTime<=NOW() And ({classid}='' Or a.ContentClassID = {classid}) And ({key}='' Or a.ContentTitle Like {likeKey}) Order By AnnounceTime Desc")
If YQasp.Has(rs) Then
  ...
  While Not rs.Eof
                ...
                <tr>
                  ...
                  <td><%= YQasp.Str.Format("<a href=""?classid={ContentClassID}"">[ {ContentClassName:EHtmlEncode(%s)} ]</a>", rs)%></td>
                  <td><%= YQasp.Str.HtmlEncode(rs("ContentTitle"))%></td>
                  <td><%= YQasp.Date.Format(rs("AnnounceTime"), "y-mm-dd")%></td>
                  <td>
...
YQasp.Db.Close(rs)
      ...
      <footer>
        <div class="row">
          <p class="col-sm-4">&copy; YunqASP v3, 2014</p>
    ...
    <script type="text/javascript">
      $(document).ready(function(){
        $("#btn-sumbit").on("click",function(){
          if(YQasp.validation("#title,#classid,#announcetime,#content"))
            $("#form-addnew").submit();
        });
        ...
        $(".btn-edit-info").on("click",function(){
          ...
          YQasp.edit($(this).attr('infoid'));
        });
      });
      var YQasp = window.YQasp = {
