...
<%
'YQasp.Console YQasp.Db.GetConn.Provider
'YQasp.Console.ShowSqlTime = False
Dim result
Select Case LCase(YQasp.Var("action"))
  Case "savenew"  '添加新内容
    '在这里编写你的服务端表单验证代码（此实例省略）
    '此处仅作简单的空值判断
    If YQasp.IsN(YQasp.Var("post.classid")) Or YQasp.IsN(YQasp.Var("post.name")) Then
      YQasp.RR "?info=adderror"
    End If
    '添加到数据库
    result = YQasp.Db.InsBatch("EC_ContentClass", "ContentClassID:{yqasp.newId}, ContentClassName:{name}, ParentID:{classid}, SortLevel:{sort:int}")
    If result>0 Then YQasp.RR "?info=addsuccess"
End Select
...
<html lang="en">
  <head>
    ...
    <title>Access数据库批量添加示例 - YunqASP测试用例中心</title>
    <!-- Bootstrap -->
