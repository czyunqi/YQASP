<!--#include file="../../code/yqasp.asp" --><!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
  <meta charset="utf-8" />
  <title>YunqASP 压缩解压演示</title>
  <style>
...
<body>
  <fieldset>
    <legend>YunqASP 无损压缩(.tar格式)演示</legend>
...
网站：<%=YQasp.GetUrl(-1)%>
备份时间：<%=YQasp.Date.Format(Now(), "y-mm-dd hh:ii:ss")%></textarea></div>
        <div>在压缩包内生成一个空文件夹：<br /><input type="text" name="newfolder" id="newfolder" value="中文文件夹/这是空的/" /></div>
        <div>压缩包保存位置：<br /><input type="text" name="tarfile" id="tarfile" value="/bakup/备份_<%=YQasp.Date.Format(Now, "ymmddhhiiss")%>.tar" /></div>
